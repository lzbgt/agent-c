...
export function runSessionStoreMigrations(db: Database): void {
  initializeSchema(db);
  ensureWorkerPortColumn(db);
  ensurePromptTrackingColumns(db);
  removeSessionSummariesUniqueConstraint(db);
  addObservationHierarchicalFields(db);
  makeObservationsTextNullable(db);
  createUserPromptsTable(db);
  ensureDiscoveryTokensColumn(db);
  createPendingMessagesTable(db);
  renameSessionIdColumns(db);
  repairSessionIdColumnRename(db);
  addFailedAtEpochColumn(db);
  addOnUpdateCascadeToForeignKeys(db);
}
...
function initializeSchema(db: Database): void {
  ...
  const appliedVersions = db.prepare('SELECT version FROM schema_versions ORDER BY version').all() as SchemaVersion[];
  ...
  if (maxApplied === 0) {
    ...
    db.prepare('INSERT INTO schema_versions (version, applied_at) VALUES (?, ?)').run(4, new Date().toISOString());
 ...
 */
function ensureWorkerPortColumn(db: Database): void {
  // Check if migration already applied
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(5) as SchemaVersion | undefined;
  if (applied) return;

  // Check if column exists
  const tableInfo = db.query('PRAGMA table_info(sdk_sessions)').all() as TableColumnInfo[];
  ...
  // Record migration
  db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(5, new Date().toISOString());
}

/**
 * Ensure prompt tracking columns exist (migration 6)
 */
function ensurePromptTrackingColumns(db: Database): void {
  // Check if migration already applied
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(6) as SchemaVersion | undefined;
  if (applied) return;

  // Check sdk_sessions for prompt_counter
  const sessionsInfo = db.query('PRAGMA table_info(sdk_sessions)').all() as TableColumnInfo[];
  ...
  const observationsInfo = db.query('PRAGMA table_info(observations)').all() as TableColumnInfo[];
  ...
  const summariesInfo = db.query('PRAGMA table_info(session_summaries)').all() as TableColumnInfo[];
  ...
  // Record migration
  db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(6, new Date().toISOString());
}

/**
 * Remove UNIQUE constraint from session_summaries.memory_session_id (migration 7)
 */
function removeSessionSummariesUniqueConstraint(db: Database): void {
  // Check if migration already applied
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(7) as SchemaVersion | undefined;
  if (applied) return;

  // Check if UNIQUE constraint exists
  const summariesIndexes = db.query('PRAGMA index_list(session_summaries)').all() as IndexInfo[];
  const hasUniqueConstraint = summariesIndexes.some(idx => idx.unique === 1);

  if (!hasUniqueConstraint) {
    // Already migrated (no constraint exists)
    db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(7, new Date().toISOString());
    return;
  }
 ...
 */
function addObservationHierarchicalFields(db: Database): void {
  // Check if migration already applied
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(8) as SchemaVersion | undefined;
  if (applied) return;

  // Check if new fields already exist
  const tableInfo = db.query('PRAGMA table_info(observations)').all() as TableColumnInfo[];
  const hasTitle = tableInfo.some(col => col.name === 'title');

  if (hasTitle) {
    // Already migrated
    db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(8, new Date().toISOString());
    return;
  }
 ...
 */
function makeObservationsTextNullable(db: Database): void {
  // Check if migration already applied
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(9) as SchemaVersion | undefined;
  if (applied) return;
  ...
  if (!textColumn || textColumn.notnull === 0) {
    ...
    db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(9, new Date().toISOString());
    return;
  }
 ...
 */
function createUserPromptsTable(db: Database): void {
  // Check if migration already applied
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(10) as SchemaVersion | undefined;
  if (applied) return;

  // Check if table already exists
  const tableInfo = db.query('PRAGMA table_info(user_prompts)').all() as TableColumnInfo[];
  if (tableInfo.length > 0) {
    // Already migrated
    db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(10, new Date().toISOString());
    return;
  }
 ...
 */
function ensureDiscoveryTokensColumn(db: Database): void {
  // Check if migration already applied to avoid unnecessary re-runs
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(11) as SchemaVersion | undefined;
  if (applied) return;
  ...
  db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(11, new Date().toISOString());
}
 ...
 */
function createPendingMessagesTable(db: Database): void {
  // Check if migration already applied
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(16) as SchemaVersion | undefined;
  if (applied) return;

  // Check if table already exists
  const tables = db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='pending_messages'").all() as TableNameRow[];
  if (tables.length > 0) {
    db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(16, new Date().toISOString());
    return;
  }
 ...
 */
function renameSessionIdColumns(db: Database): void {
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(17) as SchemaVersion | undefined;
  if (applied) return;
  ...
  const safeRenameColumn = (table: string, oldCol: string, newCol: string): boolean => {
    ...
    if (hasOldCol) {
      ...
      db.run(`ALTER TABLE ${table} RENAME COLUMN ${oldCol} TO ${newCol}`);
  ...
  // Record migration
  db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(17, new Date().toISOString());
 ...
 */
function repairSessionIdColumnRename(db: Database): void {
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(19) as SchemaVersion | undefined;
  if (applied) return;

  // Migration 17 now handles all column rename cases idempotently.
  // Just record this migration as applied.
  db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(19, new Date().toISOString());
}
 ...
 */
function addFailedAtEpochColumn(db: Database): void {
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(20) as SchemaVersion | undefined;
  if (applied) return;
  ...
  if (!hasColumn) {
    db.run('ALTER TABLE pending_messages ADD COLUMN failed_at_epoch INTEGER');
    logger.debug('DB', 'Added failed_at_epoch column to pending_messages table');
  }

  db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(20, new Date().toISOString());
}
 ...
 */
function addOnUpdateCascadeToForeignKeys(db: Database): void {
  const applied = db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(21) as SchemaVersion | undefined;
  if (applied) return;
  ...
  try {
    ...
    const hasFTS = (db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='observations_fts'").all() as { name: string }[]).length > 0;
    if (hasFTS) {
      db.run(`
    ...
    db.run('DROP TRIGGER IF EXISTS session_summaries_ai');
    db.run('DROP TRIGGER IF EXISTS session_summaries_ad');
    db.run('DROP TRIGGER IF EXISTS session_summaries_au');
    ...
    const hasSummariesFTS = (db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='session_summaries_fts'").all() as { name: string }[]).length > 0;
    if (hasSummariesFTS) {
      db.run(`
    ...
    // Record migration
    db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(21, new Date().toISOString());
