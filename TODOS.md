...
- 2026-03-20: extracted consensus runtime stop/activate control into `edge_consensus_runtime_control.cpp`, so endpoint start/stop paths now share persistence, registry activation, startup-confirmation cleanup, and stop finalization semantics with direct unit proof.
- 2026-03-20: renamed the shared managed-consensus execution surface to transport-neutral `EdgeConsensusRuntimeConfig` / `EdgeConsensusRuntimeHooks`, so the core loop, local transport, backend launch, and host helper no longer frame the builtin/native lane as HTTP-specific infrastructure.
- 2026-03-20: extracted the runtime-to-node-loop bridge into `edge_consensus_runtime_loop_adapter.cpp`, so self identity, loop-policy shaping, and structured final result JSON are shared separately from the poll/process transport loop before deeper embedded adoption.
...
- [x] W=5 — Audio streaming foundation: define WebRTC/Opus signaling + broker signaling relay endpoints (spec + broker signaling endpoints + loopback + docker-postgres smoke tests + agentd loopback tool/test added).
...
- [ ] Audio streaming: harden the shipped agentd-native WebRTC media backend after full-duplex native-plugin proof.
  - Current acceptance target (2026-04-11): native media is no longer only signaling/control glue. After the compatibility, outbound-payload helper, embedded-status helper, RTP/RTCP packet-accounting helper, graduation-criteria, and macOS M2 gate-execution slices, the builtin native-plugin lane is lab-default-eligible on the checked host and remains explicitly opt-in globally until the documented gate passes on each target release platform.
  - [x] Add broader browser/codec/candidate compatibility coverage beyond the current Chromium full-duplex PCMU/PCMA/Opus smoke path, with explicit unsupported-edge fixtures.
    - 2026-04-11: added deterministic native media compatibility fixtures for unsupported Opus channel/rate variants, unmapped dynamic RTP payloads, codec-name case normalization, and ICE relay-versus-srflx/prflx candidate handling (`tests/test_session_voice_builtin_sdp_answer.cpp`, `tests/test_session_voice_audio_decode.cpp`, `tests/test_session_voice_audio_encode.cpp`, `tests/test_session_voice_sdp_candidate.cpp`), plus outbound encoder codec-name normalization (`daemon/src/session_voice_audio_encode.cpp`).
  - [x] Continue reducing the embedded transport provider surface by extracting cohesive media/transport helpers while preserving the direct native-plugin and browser full-duplex smokes.
    - 2026-04-11: extracted outbound audio payload negotiation into a standalone helper with unit coverage (`daemon/src/session_voice_builtin_audio_payload.*`, `tests/test_session_voice_builtin_audio_payload.cpp`), leaving the native-plugin ABI and existing embedded provider load path unchanged.
  - [x] Continue the next provider-surface reduction slice by extracting either progress-event/status JSON snapshot construction or RTP/RTCP packet accounting helpers while preserving the direct native-plugin and browser full-duplex smokes.
    - 2026-04-11: extracted embedded progress/status JSON snapshot construction into a standalone helper with unit coverage (`daemon/src/session_voice_builtin_embedded_status.*`, `tests/test_session_voice_builtin_embedded_status.cpp`), reducing the embedded provider implementation to 1725 lines while preserving the native-plugin ABI and embedded provider loader/answer coverage.
  - [x] Continue the next provider-surface reduction slice by extracting RTP/RTCP packet accounting helpers while preserving the direct native-plugin and browser full-duplex smokes.
    - 2026-04-11: extracted RTP/RTCP packet accounting and sender/receiver-report cadence decisions into a standalone helper with unit coverage (`daemon/src/session_voice_builtin_packet_accounting.*`, `tests/test_session_voice_builtin_packet_accounting.cpp`), reducing the embedded provider implementation to 1620 lines while preserving embedded provider load/status coverage.
  - [x] Define production graduation criteria for `runtime_kind=builtin` + `audio_webrtc.builtin_mode=native_plugin` versus the bundled browser peer path, including operator defaults, dependency checks, and fail-closed fallback behavior.
    - 2026-04-11: documented production/lab graduation gates, operator default policy, dependency probes, smoke requirements, and fail-closed rollback rules in `docs/VOICE_NATIVE_PLUGIN_GRADUATION.md`; the production-safe default remains bundled until those gates pass for a release artifact.
  - [x] Execute the documented builtin-vs-bundled graduation gate on the current macOS M2 host before changing defaults.
    - 2026-04-11: macOS M2 gate execution passed with native stack probe, ABI/provider inspection, full build, native-plugin unit/provider tests, DTLS/SRTP + RTCP tests, and four non-skipped runtime/browser smokes (`agentd_audio_webrtc_peer_smoke`, `agentd_session_voice_webrtc_peer_runtime_smoke`, `agentd_session_voice_webrtc_peer_builtin_native_plugin_smoke`, `agentd_session_voice_webrtc_peer_builtin_default_native_plugin_smoke`). Evidence is recorded in `docs/VOICE_NATIVE_PLUGIN_GRADUATION.md`.
  - [ ] Execute the same non-skipped graduation gate on any additional target release platforms before promoting builtin under auto/default policy outside this macOS M2 host.
  ...
  - 2026-04-10: builtin `runtime_kind=builtin` now ships two real experimental daemon-managed backend lanes instead of a reserved placeholder: `signaling_stub` for in-process signaling/session orchestration, and `native_plugin` for a loadable process-local media-engine provider shared library.
  - 2026-04-10: the builtin native-provider seam is now self-describing and inspectable: runtime/config metadata expose `builtin_native_probe` and `peer.native_media_provider`, agentd ships an offline inspection helper (`tools/inspect_voice_media_provider.py`), and coverage now includes pollable ABI-v3 providers plus ABI-v1 compatibility fallback.
  - 2026-04-11: the native-plugin lane now also has an optional dependency-backed embedded transport provider target when `libjuice + libsrtp2 + usrsctp` are available locally. Coverage now includes direct provider tests plus the builtin native-plugin smoke against that provider, and the stack-checker/research docs now reflect the real installed dependency surface instead of the pre-install baseline.
  - 2026-04-11: builtin native providers now expose a pollable ABI-v4 seam, agentd drains queued provider progress events between broker-signal timeouts, and that same seam now supports bounded PCM handoff from the provider into agentd-owned memory instead of leaving decoded audio staged only inside the provider.
  ...
  - 2026-03-20: borrowed-vs-auto-create broker-session intent is now a shared C++ plan (`session_voice_broker_plan`) reused by builtin contract shaping, startup-sequence staging, and media/planned-runtime previews, with direct unit proof plus runtime smoke coverage for both builtin auto-create and borrowed-session contracts.
  - 2026-03-20: builtin not-implemented failures now derive both `builtin_start_contract` and the top-level planned `peer` preview from one shared preview builder, with direct unit proof that the backend preview exactly matches `builtin_start_contract.planned_runtime`.
  - 2026-04-10: the builtin voice backend now has an experimental in-process `signaling_stub` mode (`audio_webrtc.builtin_mode=signaling_stub` or `AGENTD_AUDIO_WEBRTC_BUILTIN_MODE=signaling_stub`) instead of being only a reserved `501` path; it reuses the shared native broker-session/signaling helpers, persists a real builtin runtime snapshot, answers remote offers with a stub SDP answer, tracks remote ICE/`bye`, and is covered by direct unit tests plus `tests/agentd_session_voice_webrtc_peer_builtin_smoke.sh`.
  - 2026-04-10: safe daemon config/runtime metadata for the managed voice lane now explicitly expose `builtin_mode=disabled|signaling_stub|native_plugin` plus `builtin_native_library_path_configured`, so operators can distinguish “native backend intentionally disabled” from “native provider configured but unloadable” and “enabled but still not the full media engine”.
  - 2026-04-10: extracted a dedicated builtin media-engine seam (`session_voice_builtin_media_engine`) so signaling/session/runtime control no longer depends on stub-specific service code; planned/live runtime metadata now report `media_engine_kind` plus `native_media_supported` / `native_media_active`, making the remaining gap purely the native RTP/media engine implementation.
  - 2026-04-10: builtin runtime observability is now also explicit instead of log-scrape-only: normalized JSONL events and persisted/runtime status expose `media_engine_state`, last transition time, and cumulative offer/answer/candidate/bye counters, so the future native RTP engine can plug into an existing operator-facing status contract.
  - 2026-04-10: builtin runtime now also supports `audio_webrtc.builtin_mode=native_plugin` with `audio_webrtc.builtin_native_library_path`, loading a process-local media-engine provider shared library through a stable C ABI; direct unit tests and a dedicated runtime smoke prove the real native-provider load path with a compiled mock provider.
  - 2026-04-11: promoted that v2 provider path onto a daemon-owned shipped sample module (`agentd_voice_builtin_media_engine_sample`) instead of a test-only fixture, corrected the shipped provider to report `native_media_supported=false` / `native_media_active=false` until a real RTP engine exists, and added `tools/check_voice_native_media_stack.py` plus refreshed dependency research to pin the next backend step to factual local package availability.
  - 2026-04-11: the optional embedded transport provider now also gathers real libjuice candidates before answering and direct coverage proves a candidate-bearing native-plugin answer plus post-answer remote-candidate transport progression against a local libjuice loopback peer; the remaining gap is still the actual DTLS/RTP media plane, not more signaling/control glue.
  - 2026-04-11: the embedded transport provider now also generates an ephemeral DTLS identity and mirrors browser-style media offers into an inactive answer with `a=setup:passive` plus a surfaced SHA-256 fingerprint; planned/live runtime snapshots persist the provider DTLS diagnostics, and the remaining gap after that point became actual DTLS/SRTP/RTP termination rather than answer shaping.
  - 2026-04-11: the embedded provider now also carries an in-process OpenSSL DTLS server/config path and surfaces handshake/export telemetry (`dtls_handshake_ready`, `dtls_exporter_ready`, selected SRTP profile, packet counts). Direct `session_voice_builtin_dtls_transport_tests` now prove DTLS 1.2 handshake plus `SRTP_AES128_CM_SHA1_80` negotiation and DTLS-SRTP exporter success in-tree, so the remaining gap is now SRTP context/RTP media termination rather than DTLS configuration itself.
  - 2026-04-11: the DTLS/SRTP proof lane now also derives real inbound/outbound libsrtp contexts from the negotiated exporter output and successfully round-trips a sample RTP packet in `session_voice_builtin_dtls_transport_tests`; the embedded provider surfaces `srtp_contexts_ready`, `srtp_inbound_ready`, `srtp_outbound_ready`, and `srtp_last_error`, so the remaining gap is now actual RTP media termination and audio pipeline ownership inside agentd rather than DTLS/SRTP session setup.
  - 2026-04-11: the embedded transport provider now terminates inbound SRTP/RTP packets far enough to parse RTP metadata and persist receive-side media counters (`rtp_packets_received`, `rtp_payload_bytes_received`, last payload type / sequence / timestamp / SSRC) through the runtime contract. That makes `native_media_supported=true` truthful for the embedded provider while `native_media_active` still only flips once live RTP is observed; the remaining gap is the minimal in-process audio path after packet termination, not DTLS/SRTP setup.
  - 2026-04-11: the embedded transport provider now also owns a first minimal receive-side audio path: it maps RTP payload types from the remote SDP, decodes `PCMU` / `PCMA` directly and `OPUS` through `libopus` when available, stages bounded PCM samples in-process, and persists audio decode/runtime counters (`audio_frames_decoded`, `audio_pcm_samples_decoded`, `audio_pcm_samples_buffered`, last codec / sample-rate / channel metadata). The remaining gap is now the next step after decode/staging: a fuller bidirectional audio/render/process pipeline rather than receive-side codec ownership.
  - 2026-04-11: the builtin service layer now also becomes a minimal audio owner for the embedded transport provider: decoded PCM can be drained out of provider staging through ABI-v4 into agentd-owned bounded memory, and runtime state now persists both provider-buffered and agentd-owned PCM counters (`audio_drain_events_total`, `audio_pcm_samples_drained_total`, `audio_pcm_samples_owned`, `audio_last_drain_samples`). The remaining gap is no longer agentd-side PCM ownership; it is using that owned PCM for actual render/process/transmit behavior.
  - 2026-04-11: agentd now also consumes the owned PCM queue through a bounded in-process monitor stage in the builtin service loop, persisting real process telemetry (`audio_process_events_total`, `audio_pcm_samples_processed_total`, `audio_last_process_samples`, `audio_last_peak_abs_pcm16`, `audio_last_rms_pcm16`) instead of only handoff counters. The remaining gap is now narrower again: a true local render/process/transmit path for the consumed PCM, not just queue ownership or audio-level monitoring.
  - 2026-04-11: agentd now also renders the consumed builtin PCM into a bounded rolling runtime-local WAV snapshot (`audio_recent.wav`) and persists render telemetry (`audio_render_events_total`, `audio_pcm_samples_rendered_total`, `audio_render_window_samples`, `audio_last_render_samples`, `audio_render_wav_path`, `audio_render_last_error`). The remaining gap is now narrower again: a true local playback or outbound-media path rather than only file rendering.
  - 2026-04-11: agentd now also has an opt-in local playback sink for that builtin PCM path (`audio_webrtc.builtin_local_playback=true` or `AGENTD_AUDIO_WEBRTC_BUILTIN_LOCAL_PLAYBACK=1`), backed by PortAudio default-output playback plus runtime telemetry (`audio_playback_events_total`, `audio_pcm_samples_played_total`, `audio_pcm_samples_playback_queued`, `audio_last_playback_samples`, `audio_playback_device_name`, `audio_playback_last_error`). The next remaining media gap is now outbound-media ownership rather than receive-side playback.
  - 2026-04-11: the native-provider seam now has ABI v5 `submit_audio`, and agentd submits processed owned PCM back into the embedded provider. The provider now encodes a bounded PCMU RTP frame, protects it with the outbound libsrtp context, transmits it over libjuice, and persists outbound media telemetry (`rtp_packets_sent`, `rtp_payload_bytes_sent`, last sent payload/sequence/timestamp/SSRC, `audio_outbound_frames_sent`, `audio_pcm_samples_submitted_total`, `audio_last_outbound_samples`, `audio_outbound_last_error`). The next remaining media gap is now negotiated codec/RTCP/full-duplex behavior rather than basic outbound RTP ownership.
  - 2026-04-11: outbound builtin media now negotiates its G.711 payload from the remote SDP instead of blindly assuming payload type 0. The embedded provider prefers negotiated `PCMU/8000/1`, falls back to negotiated `PCMA/8000/1`, records the selected outbound payload/codec/rate/channel fields in runtime telemetry, and fails outbound submit explicitly when the remote offer only exposes unsupported outbound codecs. The remaining gap is now RTCP/full-duplex behavior and negotiated Opus transmit, not basic outbound payload selection.
  - 2026-04-11: outbound builtin media now prefers negotiated `OPUS/48000/1|2` when `libopus` is available at build time, falls back to negotiated G.711 otherwise, and encodes a bounded 20 ms Opus or PCMU/PCMA RTP frame before SRTP protect/transmit. The remaining media gap is now RTCP/full-duplex behavior rather than basic RTP, payload selection, or Opus transmit ownership.
  - 2026-04-11: outbound builtin media now also emits a reduced-size RTCP Sender Report (`PT=200`) after successful RTP sends, protects it with SRTCP, transmits it over libjuice, and persists inbound/outbound RTCP counters plus last-packet metadata (`rtcp_packets_received`, `rtcp_packets_sent`, `rtcp_last_packet_type`, `rtcp_last_sent_packet_type`, SSRCs, byte counters, and `rtcp_last_error`). At that point, the remaining media gap became RTCP receiver-report/full-duplex behavior rather than basic RTP/Opus/Sender Report transmit.
  - 2026-04-11: outbound builtin media now also emits bounded RTCP Receiver Reports (`PT=201`) after inbound RTP arrives, protects them with SRTCP, transmits them over libjuice, and persists report telemetry (`rtcp_receiver_reports_sent`, `rtcp_receiver_report_blocks_sent`, reported SSRC, fraction/cumulative loss, highest extended sequence, jitter, `LSR`, and `DLSR`). The remaining media gap is now fuller compound RTCP cadence and broader browser-peer full-duplex validation, not one-shot Sender/Receiver Report ownership.
  - 2026-04-11: RTCP Sender/Receiver Report packet formatting and Receiver Report stats tracking are now extracted from the oversized embedded provider into the shared `session_voice_rtcp_report` helper, with direct rollover/loss/DLSR packet-shape coverage in `session_voice_rtcp_report_tests`. This reduces the provider's RTCP surface area while preserving the same runtime contract; the remaining media gap is still compound RTCP cadence and broader browser-peer full-duplex validation.
  - 2026-04-11: outbound RTCP Sender Reports are now cadence-bounded instead of emitted after every outbound RTP frame: the embedded provider sends the first report, then gates subsequent reports by RTP-packet count or elapsed time. The loopback provider test now proves a second immediate outbound audio submit sends another RTP packet without over-emitting a second Sender Report. The remaining media gap is now compound RTCP packet emission and broader browser-peer full-duplex validation.
  - 2026-04-11: outbound builtin RTCP now emits compound packets instead of bare reduced-size report packets: Sender Reports and Receiver Reports are followed by SDES/CNAME, `protect_outbound_rtcp_packet` now validates compound RTCP members, and loopback/native tests prove protected compound SRTCP is observed by the remote peer. The remaining media gap is now broader browser-peer full-duplex validation rather than packet-format ownership.
  - 2026-04-11: browser-style builtin answers no longer force the media section inactive once the embedded provider has DTLS identity and native RTP/SRTP support; the shared `session_voice_builtin_sdp_answer` helper now emits direction-compatible active answers (`sendrecv`, `sendonly`, `recvonly`, or `inactive` only when the offer is inactive), strips libjuice-only ICE markers that Chromium rejects, and signals the provider-owned outbound `msid`/SSRC. The native-plugin smoke now uses headless Chromium to prove browser-peer full-duplex media (`outboundPacketsSent > 0`, `inboundPacketsReceived > 0`, agentd RTP receive/send counters, audio decode, outbound frames, and RTCP send) against the embedded provider. The remaining media gap is broader hardening around browser/codec/candidate edge cases and reducing the still-large embedded provider surface, not basic full-duplex proof.
  - 2026-04-11: outbound RTP audio payload selection and 20 ms Opus/G.711 framing are now extracted from the oversized embedded provider into the shared `session_voice_audio_encode` helper with direct `session_voice_audio_encode_tests` coverage. This also fixes the outbound codec preference to choose negotiated `OPUS/48000/1|2` when available even if G.711 appears earlier in the remote SDP; the remaining gap is still broader browser/codec/candidate hardening and further provider-surface reduction.
  - 2026-04-11: the generic managed voice runtime path now also proves launchable builtin defaults, not only explicit `runtime_kind=builtin` starts: `tests/agentd_session_voice_webrtc_peer_builtin_default_native_plugin_smoke.sh` configures `AGENTD_AUDIO_WEBRTC_DEFAULT_RUNTIME_KIND=builtin` plus the embedded `native_plugin` provider, starts `voice_webrtc_peer` without a request-level backend selector, verifies config/provider availability, and completes a minimal managed offer/answer/bye lifecycle through the embedded provider. The same smoke now also persists those builtin defaults through `/api/v1/config/update`, restarts agentd without the audio env defaults, and proves the config-backed default start path launches the embedded provider too.
  - 2026-04-11: ephemeral DTLS identity generation is now extracted from the embedded provider into `session_voice_dtls_identity`, with direct `session_voice_dtls_identity_tests` proof for usable keypair/fingerprint/subject output. This keeps the provider focused on runtime transport state while preserving the active-answer DTLS contract.
  - 2026-04-11: async media progress coalescing state is now extracted from the embedded provider into `session_voice_builtin_progress_key`, with direct equality coverage in `session_voice_builtin_progress_key_tests`; this brings `session_voice_builtin_media_engine_embedded_transport_provider.cpp` back below the repo's 2000-line refactor threshold while preserving runtime event coalescing semantics.
  - 2026-04-11: browser SDP answer hardening now also covers candidate and mixed-mline edge cases: `session_voice_builtin_sdp_answer` normalizes bare local `candidate:` lines into browser-safe `a=candidate:` answer lines, lowercases the transport token, and keeps provider-owned audio `msid`/SSRC signaling scoped to audio m-lines only when a browser offer also contains non-audio sections.
  - 2026-04-11: ICE candidate normalization is now shared in `session_voice_sdp_candidate`; broker signal parsing accepts browser empty-candidate end markers as `a=end-of-candidates`, normalizes bare `candidate:` lines once, and the embedded provider falls back to normalized candidate text if libjuice rejects the original remote candidate string.
  - 2026-04-11: browser audio answers now prune unsupported audio payloads instead of mirroring the whole offer: `session_voice_builtin_sdp_answer` keeps negotiated `OPUS/48000/1|2` only when this build has `libopus`, always keeps supported PCMU/PCMA G.711, drops non-audio payloads such as telephone-event, and rejects an audio m-line as inactive/port-zero when no supported codec remains.
  - 2026-04-11: browser SDP answers now also reject unsupported non-audio m-lines such as datachannels as inactive/port-zero, strip their SCTP/ICE/DTLS transport attributes, and filter their mids out of active BUNDLE groups instead of answering them as provider-supported sections.
  - 2026-04-11: browser SDP answers now also reject extra audio m-lines as inactive/port-zero and filter their mids out of active BUNDLE groups, so the embedded provider advertises only the single audio stream it can actually own.
  - 2026-04-11: the native-plugin browser smoke now exercises those rejected-section paths against headless Chromium by generating an offer with one active audio track, one extra recvonly audio m-line, and one datachannel; the smoke asserts the answer rejects the unsupported sections while browser/agentd full-duplex media still converges on the single provider-owned audio stream, then sends a browser empty-candidate end marker and requires agentd to count it through the shared candidate-ingress path.
  - 2026-04-11: the native-plugin browser smoke now forwards real trickled Chromium ICE candidates to the broker instead of only relying on the gathered offer SDP, so the live native-plugin path covers actual browser candidate events plus the explicit empty-candidate end marker.
  - 2026-04-11: the native-plugin browser smoke now constrains the active Chromium audio transceiver to PCMU/PCMA when G.711 sender capabilities are exposed and requires agentd runtime telemetry to report that negotiated outbound codec, so the live browser proof is no longer default-Opus-only.
  - 2026-04-11: browser answer/provider coverage now proves accepted audio payload RTCP feedback and RTP header-extension attributes are preserved while unsupported payload-bound feedback plus remote offer `msid`/SSRC track identity are stripped before agentd adds its provider-owned audio stream identity.
  - 2026-04-11: shared candidate parsing now requires exact trimmed end-of-candidates markers; malformed `a=end-of-candidates...` prefixes stay as candidate payloads instead of silently terminating remote gathering, with direct candidate and broker signal parser coverage.
  - 2026-04-11: camera/video/audio live media policy is now enforced as P2P-only in the WebRTC signaling lane: shared SDP/candidate helpers detect `typ relay`, strip relay candidate lines from SDP descriptions, ignore broker-delivered relay candidates before they reach the media engine, and the WebUI/managed Playwright peers use empty ICE server configs plus relay-candidate filtering before sending or applying SDP/candidate signals. Broker remains a signaling relay only, not a media relay.
  - 2026-04-10: the remaining gap on this item is now narrower and factual: replace the shipped mock/loadable native provider path with a real long-lived agentd-native WebRTC media engine (actual SRTP/RTP/media stack), rather than adding more signaling/control glue.
...
- [x] Interop/attestation: finish inline certificate-chain enforcement and confidentiality beyond authenticated envelopes.
  ...
  - 2026-04-11: extracted edge-auth trust-root/cert-root/revocation bundle signing, certificate-chain verification, and HTTP handlers out of the generic config endpoint into `edge_auth_bundle_endpoints.cpp`, keeping both translation units under the repo's 2000-line refactor threshold while preserving edge-auth smoke coverage.
...
- [x] Node consensus: finish firmware-native adoption on top of the portable `agent_core` consensus rules and managed runtime.
  ...
  - 2026-03-20: consensus runtime config/effective-config/response semantics now live in a shared runtime-model module instead of only inside `edge_runtime_endpoints.cpp`, with direct unit proof for builtin-vs-external daemon-url drift, cluster-policy/trust defaulting, and drift surfacing before further embedded-runtime adoption.
  - 2026-03-20: consensus runtime persistence/recovery plus backend default/availability policy now also live in shared modules with direct unit proof, so `edge_runtime_endpoints.cpp` no longer owns corrupt-record cleanup, stale persisted builtin cleanup, or default backend metadata semantics by itself.
  - 2026-03-20: consensus external-helper argv shaping, runtime-artifact path planning, and stdout startup/live-status event semantics now also live in a shared process-plan module with direct unit proof, reducing the remaining endpoint-local glue before embedded consensus adoption.
  - 2026-03-20: consensus runtime stale-live refresh, recovered-stop finalization, bounded startup confirmation, and best-effort stop semantics now also live in a shared lifecycle module with direct unit proof, so `edge_runtime_endpoints.cpp` no longer owns the core managed-runtime timing/stop rules by itself.
  - 2026-03-20: consensus external-helper spawn and builtin local-runtime launch now also live in a shared backend module, with direct unit proof for runtime-state shaping and terminal-result application so future embedded/native backend work does not have to grow inside `edge_runtime_endpoints.cpp`.
  - 2026-03-20: the daemon-local managed-consensus transport (`post_hello`, local `CONSENSUS_FRAME` relay enqueue, local outbox poll, and consensus health persistence) now also lives in a shared local-transport module with direct DB-backed unit proof, narrowing the remaining work toward embedded/native consensus below the runtime loop itself.
  - 2026-03-20: the HTTP managed-consensus transport (`NODE_HELLO`, `CONSENSUS_FRAME`, auth/json headers, and outbox poll URL shaping) now also lives in a shared HTTP-transport module with direct request-shaping proof, so both runtime transports sit behind explicit factories before deeper native work.
  - 2026-04-11: extracted shared consensus health persistence, edge node summary projection, and manifest-bundle signing/HTTP handlers out of `edge_interop_endpoints.cpp`, reducing the endpoint translation unit below the repo's 2000-line refactor threshold while preserving runtime and membership-bundle smoke coverage.
  - 2026-04-11: moved consensus membership-bundle signing/normalization plus membership export/rotate/send handlers out of the oversized generic config endpoint into dedicated node-consensus modules, with direct membership-bundle unit proof and existing runtime/bundle smoke coverage.
  - 2026-04-11: added durable `stale_runtime_recovery_grace_ms` cluster policy so recent stale builtin managed-runtime records can be preserved as terminal `persisted_recovered` snapshots instead of immediately deleting all recovery evidence after daemon restart; proof: runtime model/store/recovery tests plus the main managed-runtime smoke.
  - 2026-04-11: moved consensus frame / membership bundle wire constants plus majority quorum math into portable `agent_core` (`agent/edge_interop.h`) and reused the quorum helper from the daemon consensus replica, giving embedded firmware the same core wire vocabulary and quorum threshold as agentd before deeper firmware-native adoption.
  - 2026-04-11: unified stale builtin managed-runtime recovery/clear behavior behind `recover_or_clear_edge_consensus_stale_builtin_record(...)`, so status/reconcile and store-load recovery now share one policy path for future long-lived recovery extensions.
  - 2026-04-11: extended the portable consensus helper seam with frame-kind validation, quorum checks, membership-epoch acceptance helpers, and UM-BMP message type constants, then reused those helpers/constants from the daemon replica, relay, runtime, and membership-bundle paths so embedded firmware adoption shares the same `CONSENSUS_FRAME`, `PLATFORM_CONSENSUS_MEMBERSHIP_BUNDLE`, `vote_request` / `vote_grant` / `leader_commit`, and epoch/member acceptance rules as agentd.
  - 2026-04-11: moved durable consensus policy timing normalization into portable `agent_core` and reused it from the daemon node loop, runtime start, runtime-config load, and membership rotation, so embedded firmware adoption shares campaign, lease, recovery, and stale-runtime grace clamps with agentd instead of reimplementing host-only bounds.
  - 2026-04-11: moved member-node-id validation and strictly-monotonic membership epoch advancement checks into portable `agent_core`, then reused them from daemon runtime parsing, membership bundle normalization/rotation/send, and the reusable node loop so future firmware-native membership policy does not fork host-only acceptance rules.
  - 2026-04-11: reused portable `agent_core` SHA-256 token and trust-epoch match helpers from daemon consensus runtime parsing/persistence and the reusable node loop, so firmware-native ports and agentd share digest/trust recovery acceptance instead of carrying parallel host-only checks.
  - 2026-04-11: durable consensus membership policies and signed membership bundles now carry immediate previous membership epoch/member-set lineage plus a bounded newest-first `membership_lineage` history, giving partitioned or restarting nodes recovery hints for the policy transitions they missed before deeper firmware-native adoption.
  ...
  - 2026-04-11: tightened those portable consensus acceptance predicates with role-identity checks: vote requests must be sent by their candidate, vote grants must come from a distinct voter, and leader commits must be sent by the claimed leader.
  - 2026-04-11: added portable leader-commit witness quorum acceptance and reused it from the daemon replica, so followers now require member/trust-valid unique vote witnesses that include the leader before adopting a commit.
  - 2026-04-11: moved candidate-commit and leader-activity observation gates into portable `agent_core`, then reused them from the daemon loop so firmware-native ports share the same "no existing leader plus quorum" commit boundary and timestamped leader-commit activity boundary.
  - 2026-04-11: moved decision-digest equality and self-plus-grant vote-count helpers into portable `agent_core`, reused them from the daemon replica, and added a core-only quorum-election proof that does not construct the host C++ replica.
  - 2026-04-11: moved consensus frame-id formatting, incoming-term advancement, and duplicate-frame drop predicates into portable `agent_core`, then reused them from the daemon replica so future firmware-native ports share deterministic frame identity and replay-state gates while mutable seen-frame storage remains host-local.
  - 2026-04-11: moved incoming-term stale detection into portable `agent_core` and reused it from the daemon vote-grant path, keeping lower/same/higher-term branching aligned for firmware-native replicas.
  - 2026-04-11: moved consensus frame-routing classification into portable `agent_core` and reused it from the daemon node loop, so firmware-native ports share the same vote-grant unicast and vote-request/leader-commit broadcast policy.
  - 2026-04-11: moved identity and consensus-frame shape validation into portable `agent_core`, then reused it from the daemon JSON parser/replica so future firmware-native ports share schema, kind, frame id, term, digest, candidate, leader, and identity checks instead of carrying host-only validators.
  - 2026-04-11: moved consensus membership-policy header validation and member-set nonempty checks into portable `agent_core`, then reused them from the daemon membership parser so firmware-native ports share schema/cluster/member-set shape gates while JSON type handling stays host-local.
  - 2026-04-11: moved consensus cluster-id equality into portable `agent_core` and reused it from daemon frame handling plus live membership adoption, so cross-cluster frame rejection and policy-bundle ignore decisions share the same id-safety-aware comparison as future firmware-native ports.
  - 2026-04-11: moved consensus cluster-size normalization and peer/member-derived cluster-size helpers into portable `agent_core`, then reused them from the daemon replica and live membership adoption path so firmware-native ports share the same minimum-one boundary before quorum math.
  - 2026-04-11: changed the daemon replica duplicate-frame cache so only semantically accepted consensus frames mark a frame id/term as seen; rejected frames can no longer suppress a later valid retransmission with the same frame id.
  - 2026-04-11: moved node-loop scheduler predicates for leader heartbeat, leader-lease expiry, lease-expiry recampaign cooldown, and campaign start/retry timing into portable `agent_core`, then reused them from the daemon loop so firmware-native ports do not have to fork deterministic scheduling arithmetic.
  - 2026-04-11: moved post-lease-expiry campaign retry biasing into portable `agent_core` and reused it from the daemon loop, so firmware-native ports preserve the same bounded retry schedule after a stale leader is expired.
  - 2026-04-11: moved the live membership-policy adoption gate into portable `agent_core`, requiring a strictly newer epoch and preserving the running node in the adopted member set before daemon or future firmware-native runtimes can switch policy in-loop.
  - 2026-04-11: managed consensus runtimes now consume delivered `PLATFORM_CONSENSUS_MEMBERSHIP_BUNDLE` outbox messages in the reusable node loop, adopt strictly-newer self-including membership policy without restart, clear stale in-loop election/leader state at the epoch boundary, and prove convergence under the adopted member/timing policy in `edge_node_consensus_loop_tests`, `edge_consensus_runtime_core_tests`, and the existing runtime/membership smokes.
  - 2026-04-11: moved consensus outbox message classification into portable `agent_core` and reused it from the managed runtime core, so firmware-native and daemon runtimes share the exact boundary for processing relayed `CONSENSUS_FRAME`, delivered `PLATFORM_CONSENSUS_MEMBERSHIP_BUNDLE`, and unrelated UM-BMP messages.
  - 2026-04-11: moved committed-decision presence detection into portable `agent_core` and reused it from daemon runtime-core commit exit plus node-loop heartbeat/campaign scheduling, keeping the whitespace-insensitive commit-state gate aligned for firmware-native ports.
  - 2026-04-11: moved campaign-decision source selection and campaign-start gating into portable `agent_core`, so firmware-native ports and the daemon loop share the configured-decision vs last-known-decision fallback plus the final decision/commit/timing gate before election start.
...
- Remote device bridges:
  - [x] reference tool server for ESP32 serial/MQTT bridges that speaks the same protocol and advertises UM‑ACDS tool schemas
    - 2026-04-12: added `tools/tool_server_esp32_bridge.py`, which reads UM-ACDS manifests, exposes device `parameters_schema` entries as provider-safe `esp32_...` agentd tools, and forwards executions as UM-BMP `TASK_ASSIGN` envelopes through dry-run proof plus optional serial/MQTT transports while preserving original dotted UM-ACDS tool names for the device protocol.
