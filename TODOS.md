...
- [ ] Audio streaming: replace the shipped managed browser-to-agentd live media runtime with an embedded long-lived agentd-native media service.
  ...
  - 2026-03-15: running `voice_webrtc_peer` starts are now only idempotent when the effective resolved runtime config still matches the live runtime; conflicting explicit or config-driven starts now return `409` with the existing peer snapshot, and the main runtime smoke proves persisted-running `runtime_kind=builtin`, config-driven default-backend, config-driven `node_bin`, and in-memory external `tone_hz` conflicts.
