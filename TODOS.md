...
- [ ] Audio streaming: replace the shipped managed browser-to-agentd live media runtime with an embedded long-lived agentd-native media service.
  ...
  - 2026-03-15: running `voice_webrtc_peer` starts are now only idempotent when explicit request fields still match the live runtime; conflicting explicit starts now return `409` with the existing peer snapshot, and the main runtime smoke proves both persisted-running `runtime_kind=builtin` conflict handling and an in-memory external `tone_hz` conflict.
