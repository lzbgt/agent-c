...
- [ ] Audio streaming: replace the shipped managed browser-to-agentd live media runtime with an embedded long-lived agentd-native media service.
  ...
  - 2026-03-15: the `external` WebRTC backend seam is now durable daemon config too (`audio_webrtc.peer_tool_path`, `audio_webrtc.node_bin`), safe `/api/v1/config` reports that seam without exposing secrets, and the runtime smoke now proves a config-backed `runtime_kind=external` start/stop path plus config-backed fail-fast startup via an invalid persisted `node_bin`.
