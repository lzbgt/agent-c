...
- [ ] Node consensus: replace the current host-managed helper with embedded / firmware-native adoption and add dynamic membership / recovery policy above the shipped frame + relay + managed-runtime layer.
  ...
  - 2026-04-11: extended the portable consensus helper seam with frame-kind validation, quorum checks, and membership-epoch acceptance helpers, then reused those helpers from the daemon replica so embedded firmware adoption shares the same `vote_request` / `vote_grant` / `leader_commit` and epoch/member acceptance rules as agentd.
