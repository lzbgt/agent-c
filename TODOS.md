...
- [ ] Audio streaming: harden the shipped agentd-native WebRTC media backend after full-duplex native-plugin proof.
  - Current acceptance target (2026-04-11): native media is no longer only signaling/control glue. After the compatibility and outbound-payload helper slices, the remaining production hardening for the builtin native-plugin lane is narrower provider-surface reduction plus explicit graduation criteria against the still-supported bundled browser peer.
  ...
  - [ ] Continue the next provider-surface reduction slice by extracting either progress-event/status JSON snapshot construction or RTP/RTCP packet accounting helpers while preserving the direct native-plugin and browser full-duplex smokes.
