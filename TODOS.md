...
- [ ] Audio streaming: replace the shipped managed browser-to-agentd live media runtime with an embedded long-lived agentd-native media service.
  ...
  - 2026-04-11: builtin native providers now expose a pollable ABI-v3 seam, agentd drains queued provider progress events between broker-signal timeouts, and the embedded transport provider uses that path to surface async libjuice/DTLS/SRTP progress instead of only reporting state during direct signaling callbacks.
