...
- [ ] Node consensus: replace the current host-managed helper with embedded / firmware-native adoption and add dynamic membership / recovery policy above the shipped frame + relay + managed-runtime layer.
  ...
  - 2026-04-11: added durable `stale_runtime_recovery_grace_ms` cluster policy so recent stale builtin managed-runtime records can be preserved as terminal `persisted_recovered` snapshots instead of immediately deleting all recovery evidence after daemon restart; proof: runtime model/store/recovery tests.
