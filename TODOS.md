...
- [ ] Audio streaming: harden the shipped agentd-native WebRTC media backend after full-duplex native-plugin proof.
  - Current acceptance target (2026-04-11): native media is no longer only signaling/control glue. After the compatibility, outbound-payload helper, embedded-status helper, and RTP/RTCP packet-accounting helper slices, the remaining production hardening for the builtin native-plugin lane is explicit graduation criteria against the still-supported bundled browser peer.
