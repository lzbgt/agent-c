...
- [ ] Audio streaming: replace the shipped managed browser-to-agentd live media runtime with an embedded long-lived agentd-native media service.
  ...
  - 2026-04-10: safe daemon config/runtime metadata for the managed voice lane now explicitly expose `builtin_mode=disabled|signaling_stub` alongside `builtin_available`, so operators can distinguish “native backend intentionally disabled” from “enabled but still not the full media engine”.
  ...
  - 2026-04-10: the remaining gap on this item is now narrower and factual: replacing the shipped browser-based RTP/media path with a real long-lived agentd-native WebRTC media engine. The signaling/session/runtime-control seam is no longer the blocker.
