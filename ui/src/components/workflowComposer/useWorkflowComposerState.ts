...
const buildRuntimeSkillSeedInputs = (skill: RuntimeSkillSummary | null): Record<string, unknown> => {
  ...
  for (const [key, rawProperty] of Object.entries(properties)) {
    if (!required.has(key) && !("default" in rawProperty)) continue;
...
export default function useWorkflowComposerState(props: WorkflowComposerProps) {
  ...
  const [runtimeSkillId, setRuntimeSkillId] = useLocalStorageState("agentui.workflowComposerRuntimeSkillId", "");
  const [runtimeSkillInputsJson, setRuntimeSkillInputsJson] = useLocalStorageState(
    "agentui.workflowComposerRuntimeSkillInputs",
    "{}",
  );
  ...
  const runtimeSkillsQuery = useQuery({
    queryKey: ["runtime_skills", props.baseUrl, props.authKey, "workflow_bundle"],
    queryFn: () => apiListRuntimeSkills(props.baseUrl, { kind: "workflow_bundle" }, props.auth),
    enabled: !!String(props.baseUrl || "").trim(),
    staleTime: 30_000,
  });
  const runtimeSkills = React.useMemo(
    () => runtimeSkillsQuery.data?.skills?.filter((skill) => skill.kind === "workflow_bundle") ?? [],
  ...
  const selectRuntimeSkill = React.useCallback(
    ...
    [runtimeSkills, setRuntimeSkillId, setRuntimeSkillInputsJson],
  );
  const resolveRuntimeSkill = useMutation({
    ...
    onSuccess: (resp) => {
      ...
      const workflowRequest = resp.materialized?.workflow_request;
      if (!workflowRequest || !isObjectRecord(workflowRequest)) {
        setRuntimeSkillError("Runtime skill did not resolve to a workflow payload.");
        return;
      }
      setComposerJson(JSON.stringify(workflowRequest, null, 2));
  ...
  return {
    ...
    runtimeSkillListError:
      runtimeSkillsQuery.error instanceof Error ? runtimeSkillsQuery.error.message : runtimeSkillsQuery.error ? String(runtimeSkillsQuery.error) : null,
