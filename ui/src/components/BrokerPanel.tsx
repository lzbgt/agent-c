...
export default function BrokerPanel(props: BrokerPanelProps) {
  ...
  const handleApprovalList = async () => {
    ...
    try {
      const resp = await apiBrokerTeamRunApprovalsList(base, teamId, runId, props.auth);
      if (resp.status >= 400) {
  ...
  const handleApprovalSubmit = async () => {
    ...
    const entry: Record<string, any> = { member_id: memberId };
    ...
    try {
      const resp = await apiBrokerTeamRunApprovalsCreate(base, teamId, runId, { approvals: [entry] }, props.auth);
      if (resp.status >= 400) {
        const err = resp.data?.error || resp.data?.err;
        throw new Error(err ? String(err) : `broker error (${resp.status})`);
      }
  ...
  const quorumRequestRows = React.useMemo(() => {
    const rows = brokerEvents.filter((ev) => ev?.type === "team_quorum_request");
    const deduped = new Map<string, BrokerEventRow>();
    for (let i = rows.length - 1; i >= 0; i -= 1) {
      const row = rows[i];
      const payload = row?.payload ?? {};
      const key = `${payload?.team_id || ""}:${payload?.team_run_id || ""}:${payload?.rule_id || ""}`;
      if (!deduped.has(key)) {
        deduped.set(key, row);
      }
    }
