...
function createConversationRpcHelpers({
  ...
  const invokeByKind = async (kind: string, args: UnknownRecord): Promise<unknown> => {
    ...
    if (kind === "entity_query") return makeEntityQuery(args);
    if (kind === "media_snapshot") return makeMediaSnapshot();
    if (kind === "location") return makeLocation();
    if (kind === "state_snapshot") return makeStateSnapshot();
    ...
    if (kind === "open_url") return makeOpenUrl(args);
    if (kind === "artifact_url") return makeArtifactUrl(args);
    if (kind === "script_eval") return makeScriptEval(args);
    if (kind === "page_eval") return makePageEval(args);
...
export async function runConversationUiActionRpc({
  atype,
  toolCallId,
  rpcId,
  rpcKind,
  ...
  const helpers = createConversationRpcHelpers({
    ...context,
    atype,
    toolCallId,
    rpcId,
  ...
  const t0 = Date.now();
  try {
    if (!rpcId || !(typeof context.sessionId === "string" && context.sessionId.trim().length > 0)) {
      throw new Error("missing session/rpc_id");
    }
    if (!rpcKind) throw new Error("missing rpc.kind");
    if (!allowClientRpcs) throw new Error("client RPC disabled by settings");
    if (sideEffectsRequested && !context.allowClientEffects) {
      throw new Error("client RPC side effects disabled by settings");
    }

    const result = await helpers.invokeByKind(rpcKind, rpcArgs);
