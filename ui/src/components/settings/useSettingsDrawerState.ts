...
export default function useSettingsDrawerState(props: SettingsDrawerProps) {
  ...
  const [moderatorPinnedStore, setModeratorPinnedStore] = useLocalStorageState<Record<string, Record<string, ModeratorEvent>>>(
  ...
  const [sandboxMountHostPath, setSandboxMountHostPath] = useLocalStorageState<string>("agentui.sandboxMountHostPath", "");
  ...
  const sandboxMountValidate = useMutation({
    mutationFn: async () => {
      ...
      if (!hostPath) throw new Error("host_path is required");
      if (!containerPath) throw new Error("container_path is required");
  ...
  const moderatorEventsTypes = React.useMemo(() => {
    ...
    if (types.length === 0) return ["moderator_directive", "moderator_task_published"];
    return types;
  ...
  const updateModeratorPinnedEvents = React.useCallback(
    (updater: Record<string, ModeratorEvent> | ((prev: Record<string, ModeratorEvent>) => Record<string, ModeratorEvent>)) => {
      ...
      setModeratorPinnedStore((prev) => {
        ...
        if (Object.keys(nextScope).length === 0) delete nextStore[moderatorPinnedScope];
        else nextStore[moderatorPinnedScope] = nextScope;
        return nextStore;
      });
    },
  ...
  const publishModeratorDirective = React.useCallback(async () => {
    ...
    try {
      ...
      const resp = await apiPostModeratorDirective(connection.effectiveBase, req, connection.daemonAuth);
  ...
  const publishModeratorTask = React.useCallback(async () => {
    ...
    try {
      ...
      if (assignees.length > 0) req.assignees = assignees;
      const resp = await apiPostModeratorTask(connection.effectiveBase, req, connection.daemonAuth);
  ...
  const showCopyNotice = React.useCallback((label: string, ok: boolean) => {
