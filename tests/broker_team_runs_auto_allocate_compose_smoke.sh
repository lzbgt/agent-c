...
if ! docker_compose_preflight "broker-team-auto-allocate"; then
  exit 77
fi
...
python3 -u - <<PY > "${LOG_DIR}/broker_team_runs_auto_allocate_stub.stdout.log" 2> "${LOG_DIR}/broker_team_runs_auto_allocate_stub.stderr.log" &
import json
...
start_compose_stack() {
  ...
  VERIFY_LOG="${LOG_DIR}/broker_team_runs_auto_allocate_compose_verify.log"
...
TEAM_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    -H "Authorization: Bearer ${OIDC_JWT}" \
    -H "Content-Type: application/json" \
    -d '{"display_name":"Team Runs Auto Allocate Smoke"}' \
...
if obj.get("status") != "succeeded":
  ...
  raise SystemExit(1)
runtime = obj.get("runtime_members") or []
if not runtime:
  print("expected runtime_members", obj, file=sys.stderr)
  raise SystemExit(1)
roles = [str(m.get("role","")) for m in runtime if isinstance(m, dict)]
if "planner" not in roles:
  print("expected planner role in runtime_members", obj, file=sys.stderr)
  raise SystemExit(1)
allocated = obj.get("auto_allocate_allocated_roles") or []
if "planner" not in allocated:
  print("expected auto_allocate_allocated_roles to include planner", obj, file=sys.stderr)
  raise SystemExit(1)
missing = obj.get("auto_allocate_missing_roles") or []
if missing:
  print("expected no missing roles", obj, file=sys.stderr)
  raise SystemExit(1)
if obj.get("auto_allocate_warning"):
  print("unexpected auto_allocate_warning", obj, file=sys.stderr)
  raise SystemExit(1)
PY

echo "broker_team_runs_auto_allocate_compose_smoke OK (stub=${STUB_BASE_HOST})"
