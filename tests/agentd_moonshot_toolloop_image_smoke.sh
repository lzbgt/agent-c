...
SESSION_ID="agentd_moonshot_toolloop_image_$(date +%s)_$RANDOM"
...
KIMI_API_KEY_CN="${MOONSHOT_KEY}" agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT}" "agentd_moonshot_toolloop_image" \
  --base-url "${BASE_URL}" \
  --model "${MODEL}" \
  --tools basic
...
import json, os
print(json.dumps({
  "prompt": "Do not call any tools. Reply with exactly: OK",
  "session_id": os.environ["SESSION_ID"],
  "tools": "basic",
...
if not obj.get("ok"):
  # Network tests can be flaky due to provider-side throttling/outages.
  ...
  raise SystemExit(1)
eff_tools = obj.get("effective_tools")
if eff_tools != "basic":
  print("expected effective_tools=basic, got", eff_tools, file=sys.stderr)
  raise SystemExit(1)
events = obj.get("events") or []
seen_prefetch = False
for ev in events:
  if isinstance(ev, dict) and ev.get("type") == "vision_prefetch":
    d = ev.get("data") or {}
    if d.get("ok") is True:
      seen_prefetch = True
      break
if not seen_prefetch:
  print("expected vision_prefetch event (ok=true) in events", file=sys.stderr)
  raise SystemExit(1)
