...
if ! docker_compose_preflight "broker-team-runtime-members-events"; then
  exit 77
fi
...
python3 -u - <<PY > "${LOG_DIR}/broker_team_runtime_members_events_stub.stdout.log" 2> "${LOG_DIR}/broker_team_runtime_members_events_stub.stderr.log" &
import json
...
start_compose_stack() {
  ...
  VERIFY_LOG="${LOG_DIR}/broker_team_runtime_members_events_verify.log"
...
TEAM_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    -H "Authorization: Bearer ${OIDC_JWT}" \
    -H "Content-Type: application/json" \
    -d '{"display_name":"Team Runtime Members Events Smoke"}' \
...
SSE_LOG="${LOG_DIR}/broker_team_runtime_members_events_sse.log"
  ...
  with opener.open(req, timeout=5) as resp:
    while time.time() - start < 20:
      ...
      try:
        ...
        continue
      if payload.get("type") != "team_runtime_members_updated":
        continue
      body = payload.get("payload") or {}
      if body.get("team_id") == team_id and body.get("team_run_id") == team_run_id:
        print("saw_runtime_members_update")
        sys.exit(0)
except Exception as exc:
  print(f"error: {exc}")

print("timeout waiting for runtime members events")
sys.exit(1)
PY
SSE_PID=$!
...
curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
  ...
  -H "Content-Type: application/json" \
  -d "${PATCH_BODY}" \
  "${BROKER_BASE}/v1/teams/${TEAM_ID}/runs/${TEAM_RUN_ID}/runtime_members" >/dev/null

wait "${SSE_PID}"
if [[ "$?" -ne 0 ]]; then
  echo "broker SSE did not emit runtime member update event" >&2
  cat "${SSE_LOG}" >&2 || true
  exit 1
fi

echo "broker_team_runtime_members_events_sse_compose_smoke OK"
