...
python3 -u - <<PY > "${LOG_DIR}/agentd_workflow_delegate_parallel_quorum_hashes_default_pointers_smoke.stub.stdout.log" 2> "${LOG_DIR}/agentd_workflow_delegate_parallel_quorum_hashes_default_pointers_smoke.stub.stderr.log" &
import json
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_delegate_parallel_quorum_hashes_default_pointers_smoke" \
  --tools none
...
delegate = {
  ...
  "aggregate": {
    "mode": "quorum_hashes",
    "quorum": 2
    # intentionally omit pointers to validate defaulting to [/assistant_text]
  },
  "attempts": [
    {"id": "same1", "request": {"prompt": "SAME"}, "expect": {"assistant_text_contains": "SAME"}},
    {"id": "same2", "request": {"prompt": "SAME"}, "expect": {"assistant_text_contains": "SAME"}},
    {"id": "diff", "request": {"prompt": "DIFF"}, "expect": {"assistant_text_contains": "DIFF"}},
  ]
}
...
if c.get("chosen") != "SAME" or int(c.get("chosen_count", 0)) != 2:
  print("unexpected /assistant_text chosen", c, file=sys.stderr)
  raise SystemExit(1)
...
echo "agentd_workflow_delegate_parallel_quorum_hashes_default_pointers_smoke OK"
