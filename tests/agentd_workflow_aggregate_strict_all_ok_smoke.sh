...
python3 -u - <<PY > "${LOG_DIR}/agentd_workflow_aggregate_strict_all_ok_smoke.stub.stdout.log" 2> "${LOG_DIR}/agentd_workflow_aggregate_strict_all_ok_smoke.stub.stderr.log" &
import json
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_aggregate_strict_all_ok_smoke" \
  --tools none
...
tasks = [
  ...
  {"task_id":"J","kind":"aggregate","depends_on":["A","B"],
   "aggregate":{"mode":"strict_all_ok","task_ids":["A","B"],"ok_pointer":"/ok","value_pointer":"/assistant_text"}},
]
...
j = by.get("J") or {}
if j.get("kind") != "aggregate" or j.get("mode") != "strict_all_ok":
  print("unexpected J", j, file=sys.stderr)
  raise SystemExit(1)
if j.get("ok") is not False:
  print("expected J ok=false", j, file=sys.stderr)
  raise SystemExit(1)
if "A" not in (j.get("not_ok_task_ids") or []):
  print("expected A in not_ok_task_ids", j.get("not_ok_task_ids"), file=sys.stderr)
  raise SystemExit(1)
PY

echo "agentd_workflow_aggregate_strict_all_ok_smoke OK"
