...
NAME="agentd_tool_server_ping_smoke"
...
# Start an OpenAI-compatible stub that forces one tool call to server_echo({text:"hello"}).
...
TOOL_SERVER_CMD="python3 -u ${SCRIPT_DIR}/tool_server_echo.py"

agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "${NAME}" \
  --tools basic \
  --tool-server-cmd "${TOOL_SERVER_CMD}" \
  --tool-server-ping-interval-ms 5 \
  --no-yolo

agentd_smoke_wait_health "${DAEMON_URL}"

# Ensure we cross the idle threshold since the initial manifest RPC.
sleep 0.05
...
for e in events:
  ...
  if d.get("tool_name") != "server_echo":
    continue
  content = d.get("content") if isinstance(d.get("content"), str) else ""
  try:
    tr = json.loads(content)
  except Exception as ex:
    print("failed to parse tool_result content:", content, ex, file=sys.stderr)
    raise SystemExit(1)
  data = tr.get("data") if isinstance(tr.get("data"), dict) else {}
  ping_count = data.get("ping_count")
  break

if ping_count is None:
  print("missing ping_count in tool_result events", events, file=sys.stderr)
  raise SystemExit(1)
if not isinstance(ping_count, int) or ping_count < 1:
  print("expected ping_count >= 1, got", ping_count, file=sys.stderr)
  raise SystemExit(1)
