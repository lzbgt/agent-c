...
HOST="127.0.0.1"
AUTH_TOKEN="agentd_workflow_edge_agent_smoke_token"
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_edge_agent_smoke" \
  --auth-token "${AUTH_TOKEN}" \
  --tools none

agentd_smoke_wait_health "${DAEMON_URL}"

NODE_ID="node_test_edge_agent_1"
CAPS_SHA="sha256:test_caps_edge_agent_1"
...
# For mode=agent, a manifest is not strictly required, but sending one keeps platform state realistic.
...
import json
tasks = [
  {"task_id":"AG","kind":"edge_invoke","max_attempts":200,
   "edge":{"node_id":"${NODE_ID}","mode":"agent","prompt":"say hello","timeout_ms":5000}}
]
...
# Wait for TASK_ASSIGN mode=agent in outbox.
outbox=""
...
for m in msgs:
  ...
  if body.get("task_id") != "${workflow_id}":
    continue
  if body.get("step_id") != "AG":
    continue
  if body.get("mode") != "agent":
    raise SystemExit(1)
  payload = body.get("payload") or {}
  if payload.get("prompt") != "say hello":
    raise SystemExit(1)
  raise SystemExit(0)
raise SystemExit(1)
PY
...
if r.get("kind") != "edge_invoke":
  print("unexpected AG kind", r.get("kind"), file=sys.stderr)
  raise SystemExit(1)
...
if edge_result.get("assistant_text") != "hello":
  print("unexpected edge_result", edge_result, file=sys.stderr)
  raise SystemExit(1)
PY
