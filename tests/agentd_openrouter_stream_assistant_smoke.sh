...
OPENROUTER_API_KEY="${OPENROUTER_KEY}" agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT}" "agentd_openrouter_stream_assistant_smoke" \
  --base-url "${BASE_URL}" \
  --model "${MODEL_PRIMARY}" \
  --tools none
...
run_with_model() {
  local model="$1"
  local session_id="agentd_stream_openrouter_$(date +%s)_$RANDOM"
