...
if ! docker_compose_preflight "broker-orchestrator-loop-events"; then
  exit 77
fi
...
python3 -u - <<PY >"${LOG_DIR}/broker_orchestrator_loop_events_stub.log" 2>&1 &
import json
...
HTTPServer(("127.0.0.1", int("${PORT_STUB}")), Handler).serve_forever()
PY
STUB_PID=$!
...
start_compose_stack() {
  ...
  VERIFY_LOG="${LOG_DIR}/broker_orchestrator_loop_events_compose_verify.log"
...
TEAM_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    ...
    -d '{"display_name":"Orchestrator Loop Events","meta":{"role_graph":[{"from_role":"planner","to_role":"executor"}]}}' \
...
curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
  -H "Authorization: Bearer ${OIDC_JWT}" \
  -H "Content-Type: application/json" \
  -d "{\"member_id\":\"m_executor_${TEAM_ID}\",\"agent_id\":\"${TEAM_AGENT_ID}\",\"role\":\"executor\",\"status\":\"active\"}" \
  "${BROKER_BASE}/v1/teams/${TEAM_ID}/members" >/dev/null
...
ORCH_LOG="${LOG_DIR}/broker_orchestrator_loop_events.log"
(
  cd "${ROOT}/broker"
...
python3 - <<PY
import json, sys
obj = json.loads(r'''${TEAM_RUN_JSON}''')
events = obj.get("goal_events") or []
handoffs = obj.get("handoff_events") or []
if not isinstance(events, list) or len(events) < 1:
  print("expected goal_events", obj, file=sys.stderr)
  raise SystemExit(1)
types = {e.get("type") for e in events if isinstance(e, dict)}
if "progress" not in types or "drift" not in types:
  print("expected progress + drift goal events", events, file=sys.stderr)
  raise SystemExit(1)
