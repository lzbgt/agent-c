...
if ! docker_compose_preflight "broker-team-role-overrides"; then
  exit 77
fi
...
python3 -u - <<PY > "${LOG_DIR}/broker_team_runs_role_overrides_stub.stdout.log" 2> "${LOG_DIR}/broker_team_runs_role_overrides_stub.stderr.log" &
import json
...
class H(BaseHTTPRequestHandler):
  ...
  def do_POST(self):
    if self.path in ("/__reset", "/v1/__reset"):
      MODELS.clear()
    ...
    MODELS.append(model if isinstance(model, str) else "")
    body = {
      "id": "cmpl_stub",
  ...
  def do_GET(self):
    if self.path not in ("/__models", "/v1/__models"):
      self.send_response(404)
      self.end_headers()
      return
...
start_compose_stack() {
  ...
  VERIFY_LOG="${LOG_DIR}/broker_team_runs_role_overrides_compose_verify.log"
...
TEAM_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    -H "Authorization: Bearer ${OIDC_JWT}" \
    -H "Content-Type: application/json" \
    -d '{"display_name":"Team Role Overrides Smoke","meta":{"role_overrides":{"planner":{"model":"role-default"}}}}' \
...
MEMBER_ID="m_planner_${TEAM_ID}"

reset_stub() {
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" -X POST "${STUB_BASE_HOST}/__reset" >/dev/null
}

fetch_models() {
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" "${STUB_BASE_HOST}/__models"
}
...
python3 - <<PY
import json, sys
obj = json.loads(r'''${MODELS_JSON}''')
models = obj.get("models") or []
if models != ["role-default"]:
  print("expected role-default model from team meta role_overrides", models, file=sys.stderr)
  raise SystemExit(1)
PY
...
team = {
  ...
  "role_overrides": {
    "planner": {"model": "role-model"}
  },
  "member_overrides": {
    "${MEMBER_ID}": {"model": "member-model"}
  }
}
print(json.dumps({"run": run, "team": team}))
PY
...
obj = json.loads(r'''${MODELS_JSON}''')
models = obj.get("models") or []
if models != ["member-model"]:
  print("expected member-model to override role-model", models, file=sys.stderr)
  raise SystemExit(1)
PY

echo "broker_team_runs_role_overrides_compose_smoke OK (stub=${STUB_BASE_HOST})"
