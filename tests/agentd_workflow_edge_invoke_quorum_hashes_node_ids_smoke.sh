...
HOST="127.0.0.1"
AUTH_TOKEN="agentd_workflow_edge_invoke_quorum_hashes_node_ids_smoke_token"
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_edge_invoke_quorum_hashes_node_ids_smoke" \
...
tasks = [
  ...
  {"task_id":"E2","kind":"edge_invoke","max_attempts":200,
   "edge":{"node_id":"${NODE_B}","tool":"ui.led.ws2812.control","args":{"action":"solid"},"timeout_ms":5000}},
  {"task_id":"J","kind":"aggregate","depends_on":["E1","E2"],
   "aggregate":{"mode":"quorum_hashes","task_ids":["E1","E2"],"quorum":2,"pointers":["/edge_result_sha256"],"node_pointer":"/edge/node_id"}},
]
...
if j.get("kind") != "aggregate" or j.get("mode") != "quorum_hashes" or j.get("ok") is not True:
  print("expected J ok quorum_hashes", j, file=sys.stderr)
  raise SystemExit(1)
nodes = j.get("nodes_by_task_id") or {}
if nodes.get("E1") != "${NODE_A}" or nodes.get("E2") != "${NODE_B}":
  print("unexpected nodes_by_task_id", nodes, file=sys.stderr)
  raise SystemExit(1)
checks = j.get("checks") or []
if not checks or checks[0].get("ptr") != "/edge_result_sha256" or checks[0].get("ok") is not True:
  print("unexpected checks", checks, file=sys.stderr)
  raise SystemExit(1)
chosen = checks[0].get("chosen") or ""
if not re.match(r"^sha256:[0-9a-f]{64}$", chosen):
  print("expected chosen sha256 token", chosen, file=sys.stderr)
  raise SystemExit(1)
PY

echo "agentd_workflow_edge_invoke_quorum_hashes_node_ids_smoke OK"
