...
AUTH_TOKEN="agentd_workflow_edge_agent_ref_payload_smoke_token"
...
python3 -u - <<PY > "${LOG_DIR}/agentd_workflow_edge_agent_ref_payload_smoke.stub.stdout.log" 2> "${LOG_DIR}/agentd_workflow_edge_agent_ref_payload_smoke.stub.stderr.log" &
import json
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_edge_agent_ref_payload_smoke" \
  --auth-token "${AUTH_TOKEN}" \
  --tools none

agentd_smoke_wait_health "${DAEMON_URL}"

NODE_ID="node_test_edge_ref_1"
CAPS_SHA="sha256:6666666666666666666666666666666666666666666666666666666666666666"
...
req_a = {"prompt":"Alpha","no_session":True,"tools":"none","base_url":stub_base,"api_key":"dummy","model":"stub","trace":False}
req_b = {"prompt":"Beta","no_session":True,"tools":"none","base_url":stub_base,"api_key":"dummy","model":"stub","trace":False}

tasks = [
  {"task_id":"A","request":req_a, "max_attempts": 1},
  {"task_id":"B","request":req_b, "max_attempts": 1},
  {"task_id":"J","kind":"aggregate","depends_on":["A","B"],
   "aggregate":{"mode":"collect","task_ids":["A","B"],"pointers":["/assistant_text"],"require_all":True}},
  {"task_id":"E","kind":"edge_invoke","depends_on":["J"],"max_attempts":200,
   "edge":{"node_id":node_id,"mode":"agent","payload":{"prompt":"noop","facts":{"$ref":"task.J.json:/collected"}},"timeout_ms":5000}}
]
...
# Wait for TASK_ASSIGN for step E and validate the payload contains embedded JSON (facts is object, not string).
outbox=""
for _ in $(seq 1 200); do
  outbox="$(curl -fsS --noproxy "*" --max-time 10 \
    -H "Authorization: Bearer ${AUTH_TOKEN}" \
    "${DAEMON_URL}/api/v1/edge/outbox?node_id=${NODE_ID}&cursor=0&limit=200")" || true
...
for m in msgs:
  ...
  if body.get("task_id") != "${workflow_id}":
    continue
  if body.get("step_id") != "E":
    continue
  payload = body.get("payload") or {}
  facts = payload.get("facts")
  if not isinstance(facts, dict):
    raise SystemExit(1)
  if "/assistant_text" not in facts:
    raise SystemExit(1)
  rows = facts.get("/assistant_text")
  if not isinstance(rows, list) or len(rows) != 2:
    raise SystemExit(1)
  seen = {r.get("task_id"): r.get("value") for r in rows if isinstance(r, dict)}
  if seen.get("A") != "Alpha" or seen.get("B") != "Beta":
    raise SystemExit(1)
  raise SystemExit(0)
raise SystemExit(1)
PY
