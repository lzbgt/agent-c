...
PEER_TOOL="${1:-}"
if [[ -z "${PEER_TOOL}" ]]; then
  echo "usage: $0 <agentd_audio_webrtc_peer.js>" >&2
  exit 2
fi
...
LOG_DIR="${ROOT}/build"
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/agentd_audio_webrtc_peer_smoke.log"
READY_FILE="${LOG_DIR}/agentd_audio_webrtc_peer_ready.json"
rm -f "${READY_FILE}"
...
POSTGRES_NAME="agentd_audio_webrtc_peer_smoke"
BROKER_PORT=""
PG_PORT=""
BROKER_PID=""
PEER_PID=""
...
cleanup() {
  if [[ -n "${PEER_PID}" ]]; then
    kill -TERM "${PEER_PID}" >/dev/null 2>&1 || true
...
CLIENT_AUTH_JSON="${LOG_DIR}/broker_audio_webrtc_client_auth.json"
...
BROKER_BIN="${LOG_DIR}/agentd-broker-audio-webrtc-peer"
(
  cd "${ROOT}/broker"
...
node "${PEER_TOOL}" \
  ...
  --deadline-ms 15000 >>"${LOG_FILE}" 2>&1 &
PEER_PID=$!
...
if [[ ! -f "${READY_FILE}" ]]; then
  echo "Peer tool did not report readiness; see ${LOG_FILE}" >&2
  exit 1
fi
...
curl -fsS --noproxy "*" --max-time 10 \
  "http://127.0.0.1:${BROKER_PORT}/v1/audio/sessions/${SESSION_ID}/signal" \
  -H "Authorization: Bearer audio-webui-token" \
...
status_after_bye="$(curl -sS --noproxy "*" --max-time 10 -o /dev/null -w '%{http_code}' \
  "http://127.0.0.1:${BROKER_PORT}/v1/audio/sessions/${SESSION_ID}" \
  -H "Authorization: Bearer audio-webui-token")"
if [[ "${status_after_bye}" != "404" ]]; then
  echo "expected deleted audio session after bye, got status ${status_after_bye}" >&2
  exit 1
fi

echo "agentd_audio_webrtc_peer_smoke OK"
