...
MODEL="${AGENT_TEST_DEEPSEEK_STREAM_TOOL_MODEL:-${AGENT_TEST_DEEPSEEK_TOOL_MODEL:-deepseek-chat}}"

HOST="127.0.0.1"
PORT="$(agentd_smoke_pick_port)"

SESSION_ID="agentd_deepseek_stream_tool_$(date +%s)_$RANDOM"
...
DEEPSEEK_API_KEY="${DEEPSEEK_KEY}" agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT}" "agentd_deepseek_stream_tool_call_smoke" \
  --base-url "${BASE_URL}" \
  --model "${MODEL}" \
  --tools basic
...
if ('"http_status":429' in s or '"status":429' in s) and ("overloaded" in lower or "rate" in lower):
  print("SKIP: DeepSeek provider overloaded (429)", file=sys.stderr)
  raise SystemExit(77)
if ('"type":"error"' in s or '"type": "error"' in s):
  if ("unsupported" in lower or "not support" in lower or "not_supported" in lower) and ("tool" in lower or "stream" in lower):
    print("SKIP: DeepSeek provider does not support tool-call streaming", file=sys.stderr)
    raise SystemExit(77)
