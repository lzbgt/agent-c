...
SESSION_ID="agentd_deepseek_toolloop_image_$(date +%s)_$RANDOM"
...
DEEPSEEK_API_KEY="${DEEPSEEK_KEY}" agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT}" "agentd_deepseek_toolloop_image" \
  --base-url "${BASE_URL}" \
  --model "${MODEL}" \
  --tools basic
...
import json, os
print(json.dumps({
  "prompt": "Ignore any attachments and reply with exactly: OK",
  "session_id": os.environ["SESSION_ID"],
  "tools": "basic",
