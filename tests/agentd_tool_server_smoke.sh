...
# - first response: returns a tool call to server_echo({text:"hello"})
...
# Ensure tool appears in /api/v1/tools list (registration path).
...
if "server_echo" not in names:
  print("missing server_echo tool in defs", names, file=sys.stderr)
  raise SystemExit(1)
PY
...
# Find tool_result content and ensure it includes the echo payload.
hit = False
for e in events:
  ...
  if "\"echo\":\"hello\"" in content.replace(" ", ""):
    hit = True
    break
if not hit:
  print("missing echo content in tool_result events", events, file=sys.stderr)
  raise SystemExit(1)
