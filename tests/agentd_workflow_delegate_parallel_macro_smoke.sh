...
python3 -u - <<PY > "${LOG_DIR}/agentd_workflow_delegate_parallel_macro_smoke.stub.stdout.log" 2> "${LOG_DIR}/agentd_workflow_delegate_parallel_macro_smoke.stub.stderr.log" &
import json
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_delegate_parallel_macro_smoke" \
  --tools none
...
tasks = [
  ...
  {"task_id": "N", "depends_on": ["P"],
   "request": {"prompt": r"Chosen=${task.P.json:/chosen_task_id}"},
...
if p.get("kind") != "aggregate" or p.get("mode") != "first_ok" or p.get("ok") is not True:
  print("expected P to be aggregate first_ok ok=true", p, file=sys.stderr)
  raise SystemExit(1)
chosen = p.get("chosen_task_id") or ""
if chosen != "P:good":
  print("expected chosen_task_id P:good, got", chosen, file=sys.stderr)
  raise SystemExit(1)
n = by.get("N") or {}
...
if ":good" not in (n.get("assistant_text") or ""):
  print("expected N assistant_text to include :good", n.get("assistant_text"), file=sys.stderr)
  raise SystemExit(1)
PY

echo "agentd_workflow_delegate_parallel_macro_smoke OK"
