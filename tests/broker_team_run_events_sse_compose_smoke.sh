...
if ! docker_compose_preflight "broker-team-run-events"; then
  exit 77
fi
...
cleanup() {
  ...
  if [[ -n "${SSE_PID:-}" ]]; then
    kill -TERM "${SSE_PID}" >/dev/null 2>&1 || true
...
python3 -u - <<PY > "${LOG_DIR}/broker_team_run_events_stub.stdout.log" 2> "${LOG_DIR}/broker_team_run_events_stub.stderr.log" &
import json
...
start_compose_stack() {
  ...
  VERIFY_LOG="${LOG_DIR}/broker_team_run_events_verify.log"
...
TEAM_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    -H "Authorization: Bearer ${OIDC_JWT}" \
    -H "Content-Type: application/json" \
    -d '{"display_name":"Team Run Events Smoke"}' \
...
SSE_LOG="${LOG_DIR}/broker_team_run_events_sse.log"
python3 -u - <<PY > "${SSE_LOG}" 2>&1 &
import json
import socket
import ssl
import sys
...
try:
  with opener.open(req, timeout=5) as resp:
    while time.time() - start < 25:
      ...
      if event_type not in ("team_run_created", "team_run_status"):
        continue
      body = payload.get("payload") or {}
      if body.get("team_id") != team_id:
        continue
      run_id = body.get("team_run_id")
      if not run_id:
        continue
      seen.setdefault(run_id, set()).add(event_type)
      if "team_run_created" in seen[run_id] and "team_run_status" in seen[run_id]:
        print("saw_team_run_events")
        sys.exit(0)
except Exception as exc:
  print(f"error: {exc}")

print("timeout waiting for team run events")
sys.exit(1)
PY
SSE_PID=$!
...
while true; do
  if grep -q '^:ok' "${SSE_LOG}" 2>/dev/null; then
    break
  fi
  if (( $(date +%s) >= READY_DEADLINE )); then
    echo "broker SSE stream did not become ready" >&2
    cat "${SSE_LOG}" >&2 || true
    exit 1
  fi
  sleep 0.2
...
if ! wait "${SSE_PID}"; then
  echo "broker SSE did not emit team run created/status events" >&2
  cat "${SSE_LOG}" >&2 || true
  exit 1
fi

echo "broker_team_run_events_sse_compose_smoke OK"
