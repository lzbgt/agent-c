...
if ! docker_compose_preflight "broker-team-quorum-events"; then
  exit 77
fi
...
python3 -u - <<PY > "${LOG_DIR}/broker_team_quorum_events_stub.stdout.log" 2> "${LOG_DIR}/broker_team_quorum_events_stub.stderr.log" &
import json
...
start_compose_stack() {
  ...
  VERIFY_LOG="${LOG_DIR}/broker_team_quorum_events_verify.log"
...
SSE_LOG="${LOG_DIR}/broker_team_quorum_events_sse.log"
...
try:
  with opener.open(req, timeout=5) as resp:
    while time.time() - start < 20:
      ...
      if etype == "team_quorum_request":
        seen_req = True
      if etype == "team_quorum_result":
        seen_res = True
      if seen_req and seen_res:
        print("saw_quorum_events")
        sys.exit(0)
except Exception as exc:
  print(f"error: {exc}")

print("timeout waiting for quorum events")
sys.exit(1)
PY
SSE_PID=$!
...
TEAM_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    -H "Authorization: Bearer ${OIDC_JWT}" \
    -H "Content-Type: application/json" \
    -d '{"display_name":"Team Quorum Events Smoke"}' \
...
wait "${SSE_PID}"
if [[ "$?" -ne 0 ]]; then
  echo "broker SSE did not emit quorum events" >&2
  cat "${SSE_LOG}" >&2 || true
  exit 1
fi

echo "broker_team_quorum_events_sse_compose_smoke OK"
