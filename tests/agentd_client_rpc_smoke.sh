...
if not has_tool_result("ui_action"):
  print("expected tool_result for ui_action; got:", ev, file=sys.stderr)
  raise SystemExit(1)

if not has_tool_result("client_wait_event"):
  print("expected tool_result for client_wait_event; got:", ev, file=sys.stderr)
  raise SystemExit(1)

def has_ui_action_probe():
  for e in ev:
