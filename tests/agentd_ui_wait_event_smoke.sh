...
SESSION_ID="agentd_ui_wait_event_smoke_$(date +%s)_$RANDOM"
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_ui_wait_event_smoke" \
  --tools host \
  --no-yolo

agentd_smoke_wait_health "${DAEMON_URL}"

# Post the client event shortly after the run starts so ui_wait_event blocks briefly.
(
  sleep 0.8
...
import json
print(json.dumps({
  "prompt": "wait for the UI event then say OK",
  "session_id": "${SESSION_ID}",
  "tools": "host",
  "yolo": False,
  "tools_root": "@host",
