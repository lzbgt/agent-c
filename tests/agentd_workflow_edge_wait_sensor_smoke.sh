...
python3 -u - <<PY > "${LOG_DIR}/agentd_workflow_edge_wait_sensor_smoke.stub.stdout.log" 2> "${LOG_DIR}/agentd_workflow_edge_wait_sensor_smoke.stub.stderr.log" &
import json
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_edge_wait_sensor_smoke" \
  --tools none
...
echo "agentd_workflow_edge_wait_sensor_smoke OK"
