...
if [[ -z "${AGENTD_BIN}" || -z "${PLUGIN_HOST_BIN}" || -z "${PLUGIN_PATH}" ]]; then
  echo "usage: agentd_tool_plugin_server_big_smoke.sh <agentd_bin> <plugin_host_bin> <plugin_path>" >&2
  exit 2
fi
              ...
              "tool_calls": [
                {
                  ...
                  "function": {
                    "name": "ext_big",
                    "arguments": json.dumps({"size": 4194305}),
                  },
                }
              ],
            },
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "${NAME}" \
  --tools host \
  --no-yolo \
  --tool-server-cmd "${PLUGIN_HOST_BIN} --plugin ${PLUGIN_PATH}"
...
import json
print(json.dumps({
  "prompt": "Call ext_big then say OK",
  "no_session": True,
  "tools": "host",
  "yolo": False,
...
tool_results = [e for e in events if isinstance(e, dict) and e.get("type") == "tool_result" and (e.get("data") or {}).get("tool_name") == "ext_big"]
if not tool_results:
  types = [e.get("type") for e in events if isinstance(e, dict)]
  print("missing ext_big tool_result; event types=", types, file=sys.stderr)
  raise SystemExit(1)
content = (tool_results[-1].get("data") or {}).get("content") or ""
if "plugin result exceeded max bytes" not in content:
  print("unexpected tool_result content:", content[:200], file=sys.stderr)
  raise SystemExit(1)
