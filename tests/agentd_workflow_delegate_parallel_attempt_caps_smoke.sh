...
python3 -u - <<PY > "${LOG_DIR}/agentd_workflow_delegate_parallel_attempt_caps_smoke.stub.stdout.log" 2> "${LOG_DIR}/agentd_workflow_delegate_parallel_attempt_caps_smoke.stub.stderr.log" &
import json
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_delegate_parallel_attempt_caps_smoke" \
  --tools none
...
att = by.get("P:good") or {}
if att.get("ok") is not True:
  print("expected P:good ok true", att, file=sys.stderr)
  raise SystemExit(1)
eff = att.get("effective_timeout_ms")
if eff != 333:
  print("expected effective_timeout_ms 333, got", eff, file=sys.stderr)
  raise SystemExit(1)
PY

echo "agentd_workflow_delegate_parallel_attempt_caps_smoke OK"
