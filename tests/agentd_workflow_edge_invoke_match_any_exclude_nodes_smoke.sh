...
HOST="127.0.0.1"
AUTH_TOKEN="agentd_workflow_edge_invoke_match_any_exclude_nodes_smoke_token"
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_workflow_edge_invoke_match_any_exclude_nodes_smoke" \
...
import json
tasks = [
  {"task_id":"E","kind":"edge_invoke","max_attempts":200,
   "edge":{
     "match_any":{"requires_tools":["ui.led.ws2812.control"],"exclude_node_ids":["${NODE_A}"]},
     "tool":"ui.led.ws2812.control","args":{"action":"solid"},"timeout_ms":5000
   }},
]
...
for m in msgs:
  ...
  if env.get("type") != "TASK_ASSIGN":
    continue
  body = env.get("body") or {}
  if body.get("task_id") == "${workflow_id}" and body.get("step_id") == "${step_id}":
    print("unexpected TASK_ASSIGN in excluded node outbox", body, file=sys.stderr)
    raise SystemExit(1)
raise SystemExit(0)
PY
}

wait_outbox_assign "${NODE_B}" "E"
assert_no_assign "${NODE_A}" "E"
...
send_edge_msg "$(task_ack "${NODE_B}" "E")"
send_edge_msg "$(task_done "${NODE_B}" "E")"
...
if edge.get("node_id") != "${NODE_B}":
  print("expected selected node_id to be NODE_B", edge, file=sys.stderr)
  raise SystemExit(1)
PY

echo "agentd_workflow_edge_invoke_match_any_exclude_nodes_smoke OK"
