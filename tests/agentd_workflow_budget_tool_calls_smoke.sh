...
if [[ -z "${AGENTD_BIN}" || -z "${PLUGIN_PATH}" ]]; then
  echo "usage: agentd_workflow_budget_tool_calls_smoke.sh <agentd_bin> <plugin_path>" >&2
  exit 2
fi
...
req = {
  ...
  "workflow_limits": {
    # Only enough budget for ONE tool call across the entire workflow.
...
if by_id.get("B", {}).get("status") != "cancelled":
  ...
  raise SystemExit(1)
if "workflow budget exceeded" not in str(by_id.get("B", {}).get("error","")):
  print("expected B error to mention budget exceeded", by_id.get("B"), file=sys.stderr)
  raise SystemExit(1)
...
if int(a.get("tool_calls_total", 0) or 0) != 1:
  print("expected A.tool_calls_total == 1", a, file=sys.stderr)
  raise SystemExit(1)
PY
