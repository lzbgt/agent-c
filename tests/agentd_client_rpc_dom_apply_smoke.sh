...
SESSION_ID="agentd_client_rpc_dom_apply_smoke_$(date +%s)_$RANDOM"
...
# - 0 tool results: call ui_action(type=client_rpc, rpc.kind=dom_apply)
# - 1 tool result: call client_wait_event(type=client_rpc_result, data_match.rpc_id=r1)
# - 2+ tool results: assistant "OK"
python3 -u - <<PY > "${LOG_DIR}/agentd_client_rpc_dom_apply_smoke.stub.stdout.log" 2> "${LOG_DIR}/agentd_client_rpc_dom_apply_smoke.stub.stderr.log" &
import json
              ...
              "tool_calls": [
                {
                  ...
                  "function": {
                    ...
                    "arguments": json.dumps({
                      "type": "client_rpc",
                      "title": "apply dom patch",
                      "rpc_id": "r1",
                      "rpc": {"kind": "dom_apply", "side_effects": True, "args": {"ops":[{"op":"create","tag":"div","parent_selector":"body","text":"hello"}]}},
                      "auto_run": True,
                    }),
                  },
                }
...
agentd_smoke_start "${AGENTD_BIN}" "${HOST}" "${PORT_DAEMON}" "agentd_client_rpc_dom_apply_smoke" \
  --tools host \
  --no-yolo

agentd_smoke_wait_health "${DAEMON_URL}"

# Synthetic result: in real life, the browser client posts this after applying dom_apply.
(
  sleep 1.0
...
print(json.dumps({
  ...
  "data": {"rpc_id":"r1","ok": True, "rpc_kind":"dom_apply", "result":{"kind":"dom_apply","applied":1,"total":1}},
...
import json
print(json.dumps({
  "prompt": "apply a dom patch via client rpc, wait for result, then say OK",
  "session_id": "${SESSION_ID}",
  "tools": "host",
  "yolo": False,
  "base_url": "${STUB_BASE}",
...
def has_ui_action_dom_apply():
  for e in ev:
    if isinstance(e, dict) and e.get("type") == "ui_action":
      ...
      if rpc.get("kind") == "dom_apply":
        return True
  return False

if not has_ui_action_dom_apply():
  print("expected ui_action client_rpc dom_apply; got:", ev, file=sys.stderr)
  raise SystemExit(1)
...
echo "agentd_client_rpc_dom_apply_smoke OK"
