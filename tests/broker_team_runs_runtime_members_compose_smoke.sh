...
if ! docker_compose_preflight "broker-team-runs-runtime-members"; then
  exit 77
fi
...
python3 -u - <<PY > "${LOG_DIR}/broker_team_runs_runtime_members_stub.stdout.log" 2> "${LOG_DIR}/broker_team_runs_runtime_members_stub.stderr.log" &
import json
...
start_compose_stack() {
  ...
  VERIFY_LOG="${LOG_DIR}/broker_team_runs_runtime_members_compose_verify.log"
...
TEAM_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    -H "Authorization: Bearer ${OIDC_JWT}" \
    -H "Content-Type: application/json" \
    -d '{"display_name":"Team Runs Runtime Members Smoke"}' \
...
PATCH_REPLACE_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    ...
    -H "Authorization: Bearer ${OIDC_JWT}" \
    -H "Content-Type: application/json" \
    -d "${PATCH_REPLACE_BODY}" \
    "${BROKER_BASE}/v1/teams/${TEAM_ID}/runs/${TEAM_RUN_ID}/runtime_members"
)"
python3 - <<PY
import json, sys
obj = json.loads(r'''${PATCH_REPLACE_JSON}''')
runtime = obj.get("runtime_members")
if not isinstance(runtime, list) or len(runtime) != 1:
  raise SystemExit(f"expected runtime_members len=1, got {runtime}")
if runtime[0].get("member_id") != "rtm_init":
  raise SystemExit(f"expected rtm_init, got {runtime}")
if obj.get("status") != "succeeded":
  raise SystemExit(f"unexpected status {obj.get('status')}")
PY

STATUS_AFTER_REPLACE="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    -H "Authorization: Bearer ${OIDC_JWT}" \
...
python3 - <<PY
import json, sys
obj = json.loads(r'''${STATUS_AFTER_REPLACE}''')
runtime = obj.get("runtime_members")
if not isinstance(runtime, list) or len(runtime) != 1:
  raise SystemExit(f"expected runtime_members len=1 after replace, got {runtime}")
PY

PATCH_MERGE_BODY="$(python3 - <<PY
import json
payload = {
  "mode": "merge",
  "runtime_members": [
...
python3 - <<PY
import json, sys
obj = json.loads(r'''${PATCH_MERGE_JSON}''')
runtime = obj.get("runtime_members")
if not isinstance(runtime, list) or len(runtime) != 2:
  raise SystemExit(f"expected runtime_members len=2, got {runtime}")
ids = {m.get("member_id") for m in runtime}
if not {"rtm_init", "rtm_merge"}.issubset(ids):
  raise SystemExit(f"missing expected ids in runtime_members: {runtime}")
PY
...
python3 - <<PY
import json, sys
obj = json.loads(r'''${STATUS_AFTER_MERGE}''')
runtime = obj.get("runtime_members")
if not isinstance(runtime, list) or len(runtime) != 2:
  raise SystemExit(f"expected runtime_members len=2 after merge, got {runtime}")
PY

PATCH_CLEAR_BODY='{"mode":"replace","runtime_members":[]}'
PATCH_CLEAR_JSON="$(
  curl -fsS -k --noproxy "*" "${CURL_BASE_OPTS[@]}" \
    -X PATCH \
    -H "Authorization: Bearer ${OIDC_JWT}" \
...
python3 - <<PY
import json, sys
obj = json.loads(r'''${PATCH_CLEAR_JSON}''')
if "runtime_members" in obj:
  raise SystemExit(f"expected runtime_members cleared, got {obj.get('runtime_members')}")
PY
...
python3 - <<PY
import json, sys
obj = json.loads(r'''${STATUS_AFTER_CLEAR}''')
if "runtime_members" in obj:
  raise SystemExit(f"expected runtime_members cleared in status, got {obj.get('runtime_members')}")
PY

echo "broker_team_runs_runtime_members_compose_smoke OK"
