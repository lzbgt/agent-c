...
PORT_DEAD="$(agentd_smoke_pick_port)"
PORT_LOCAL="$(agentd_smoke_pick_port)"
NAME="agentd_workflow_agentd_parallel_macro_smoke"
...
# Start a remote agentd instance (one collaboration target).
...
# Start a local agentd instance (runs the workflow with kind:"agentd_parallel").
#
...
tasks = [
  {
    ...
    "agentd_parallel": {
      "targets": [
        {"id": "dead", "base_url": "http://${HOST}:${PORT_DEAD}", "allow_error": True},
        {"id": "ok", "base_url": "${REMOTE_URL}", "allow_error": True}
      ],
      "agentd_call": {
...
# Derived tasks exist.
if "P:ok" not in by_id:
  print("missing derived task P:ok", file=sys.stderr)
  raise SystemExit(1)
if "P:dead" not in by_id:
  print("missing derived task P:dead", file=sys.stderr)
  raise SystemExit(1)

res = obj.get("result") or {}
by = res.get("results_by_task") or {}
j = by.get("P") or {}
if j.get("mode") != "first_ok" or j.get("ok") is not True:
  print("unexpected join result", j, file=sys.stderr)
  raise SystemExit(1)
if (j.get("assistant_text") or "").strip() != "remote ok":
  print("unexpected assistant_text", j.get("assistant_text"), file=sys.stderr)
  raise SystemExit(1)
if j.get("chosen_task_id") != "P:ok":
