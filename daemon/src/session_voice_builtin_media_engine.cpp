...
class BuiltinVoicePeerSignalingStubEngine final : public VoicePeerBuiltinMediaEngine {
 public:
  ...
  bool initialize(
    ...
    if (out_event) *out_event = make_engine_event(info(), "media_engine_initialized");
    return true;
  }
  ...
  bool handle_remote_description(
    ...
    if (out_event) {
      Json::Value payload = make_engine_event(info(), "stub_answer_ready");
  ...
  bool handle_remote_candidate(
    ...
    if (out_err) out_err->clear();
    if (out_event) {
      Json::Value payload = make_engine_event(info(), "remote_candidate_ready");
  ...
  void handle_remote_bye(
    ...
    if (!out_event) return;
    Json::Value payload = make_engine_event(info(), "remote_bye");
  ...
  void handle_local_shutdown(
    ...
    if (!out_event) return;
    Json::Value payload = make_engine_event(info(), "local_bye_sent");
