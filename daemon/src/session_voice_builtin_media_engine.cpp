...
constexpr size_t kMediaPluginAudioDrainSamples = 4096;
constexpr size_t kMediaPluginCodecNameBufBytes = 128;
...
bool validate_voice_media_plugin_bridge_v4(
  const VoiceMediaPluginApiBridge& api,
  std::string* out_err
) {
  if (!validate_voice_media_plugin_bridge_v3(api, out_err)) return false;
  if (!api.drain_audio) {
...
bool populate_probe_from_api_v4(
  const std::string& library_path,
  const agentd_voice_media_engine_provider_v4* api,
  VoiceMediaPluginProbe* out_probe,
  std::string* out_err
) {
  if (!api) {
    if (out_err) *out_err = "builtin native media engine returned null v4 API";
    return false;
  }
  if (api->abi_version != AGENTD_VOICE_MEDIA_ENGINE_PROVIDER_ABI_V4) {
    if (out_err) *out_err = "builtin native media engine ABI v4 mismatch";
    return false;
  }
  const std::string kind = trim_copy(api->media_engine_kind ? api->media_engine_kind : "");
  if (kind != "builtin_native_plugin") {
  ...
  bridge.poll_status = api->poll_status;
  bridge.drain_audio = api->drain_audio;
  if (!validate_voice_media_plugin_bridge_v4(bridge, out_err)) return false;
...
bool probe_builtin_voice_peer_native_media_engine_impl(
  ...
  if (get_api_v4) {
    VoiceMediaPluginProbe probe;
    if (!populate_probe_from_api_v4(library_path, get_api_v4(), &probe, out_err)) {
      media_plugin_close(handle);
      return false;
    }
    if (out_probe) {
...
class BuiltinVoicePeerNativePluginEngine final : public VoicePeerBuiltinMediaEngine {
 public:
  ...
  bool drain_audio(
    ...
    if (out_chunk) *out_chunk = VoicePeerBuiltinAudioChunk{};
    if (out_event) *out_event = Json::Value(Json::nullValue);
    if (!api_.drain_audio) return true;

    int16_t pcm_buf[kMediaPluginAudioDrainSamples];
    size_t pcm_samples = 0;
    int sample_rate_hz = 0;
    int channels = 0;
    int frame_samples_per_channel = 0;
    char codec_name[kMediaPluginCodecNameBufBytes];
    char event_buf[kMediaPluginEventBufBytes];
    char err_buf[kMediaPluginErrBufBytes];
    clear_c_buffer(codec_name);
    clear_c_buffer(event_buf);
    clear_c_buffer(err_buf);
    if (!api_.drain_audio(
          instance_,
          pcm_buf,
          kMediaPluginAudioDrainSamples,
          &pcm_samples,
          &sample_rate_hz,
          &channels,
      ...
      if (out_err) {
        *out_err = trim_copy(err_buf).empty()
          ? "builtin native media engine drain_audio failed"
          : trim_copy(err_buf);
      }
      return false;
    }
    if (out_chunk && pcm_samples > 0) {
      out_chunk->pcm_samples.assign(pcm_buf, pcm_buf + pcm_samples);
      out_chunk->sample_rate_hz = sample_rate_hz;
      out_chunk->channels = channels;
      out_chunk->frame_samples_per_channel = frame_samples_per_channel;
      out_chunk->codec_name = trim_copy(codec_name);
    }
    const std::string raw_event = trim_copy(event_buf);
    if (!raw_event.empty()) {
      return parse_media_plugin_event_json(event_buf, info_, out_event, out_err);
    }
    if (pcm_samples == 0) return true;
    if (out_event) {
      Json::Value payload = make_engine_event(
        info_,
        "audio_chunk_drained",
        info_.native_media_active ? "media_active" : "media_transport_ready");
      payload["audio_last_drain_samples"] = Json::Int64(static_cast<int64_t>(pcm_samples));
      if (sample_rate_hz > 0) payload["audio_last_sample_rate_hz"] = sample_rate_hz;
      if (channels > 0) payload["audio_last_channels"] = channels;
      if (frame_samples_per_channel > 0) {
        payload["audio_last_frame_samples_per_channel"] = frame_samples_per_channel;
      }
      if (codec_name[0]) payload["audio_last_codec_name"] = trim_copy(codec_name);
      *out_event = payload;
    }
    return true;
  }
...
void note_voice_peer_media_engine_event(
  ...
  if (payload.isMember("rtcp_sender_reports_sent") &&
      (payload["rtcp_sender_reports_sent"].isInt64() ||
       payload["rtcp_sender_reports_sent"].isUInt64())) {
    runtime->rtcp_sender_reports_sent = payload["rtcp_sender_reports_sent"].asInt64();
  }
  if (payload.isMember("rtcp_receiver_reports_sent") &&
      (payload["rtcp_receiver_reports_sent"].isInt64() ||
       payload["rtcp_receiver_reports_sent"].isUInt64())) {
    runtime->rtcp_receiver_reports_sent =
      payload["rtcp_receiver_reports_sent"].asInt64();
  }
  if (payload.isMember("rtcp_receiver_report_blocks_sent") &&
      (payload["rtcp_receiver_report_blocks_sent"].isInt64() ||
       payload["rtcp_receiver_report_blocks_sent"].isUInt64())) {
    runtime->rtcp_receiver_report_blocks_sent =
      payload["rtcp_receiver_report_blocks_sent"].asInt64();
  }
  ...
  if (payload.isMember("rtcp_last_reported_rtp_ssrc") &&
      (payload["rtcp_last_reported_rtp_ssrc"].isInt64() ||
       payload["rtcp_last_reported_rtp_ssrc"].isUInt64())) {
    runtime->rtcp_last_reported_rtp_ssrc =
      payload["rtcp_last_reported_rtp_ssrc"].asInt64();
  }
  if (payload.isMember("rtcp_last_report_fraction_lost") &&
      (payload["rtcp_last_report_fraction_lost"].isInt64() ||
       payload["rtcp_last_report_fraction_lost"].isUInt64())) {
    runtime->rtcp_last_report_fraction_lost =
      payload["rtcp_last_report_fraction_lost"].asInt64();
  }
  if (payload.isMember("rtcp_last_report_cumulative_lost") &&
      (payload["rtcp_last_report_cumulative_lost"].isInt64() ||
       payload["rtcp_last_report_cumulative_lost"].isUInt64())) {
    runtime->rtcp_last_report_cumulative_lost =
      payload["rtcp_last_report_cumulative_lost"].asInt64();
  }
  if (payload.isMember("rtcp_last_report_highest_sequence") &&
      (payload["rtcp_last_report_highest_sequence"].isInt64() ||
       payload["rtcp_last_report_highest_sequence"].isUInt64())) {
    runtime->rtcp_last_report_highest_sequence =
      payload["rtcp_last_report_highest_sequence"].asInt64();
  }
  if (payload.isMember("rtcp_last_report_jitter") &&
      (payload["rtcp_last_report_jitter"].isInt64() ||
       payload["rtcp_last_report_jitter"].isUInt64())) {
    runtime->rtcp_last_report_jitter = payload["rtcp_last_report_jitter"].asInt64();
  }
  if (payload.isMember("rtcp_last_report_lsr") &&
      (payload["rtcp_last_report_lsr"].isInt64() ||
       payload["rtcp_last_report_lsr"].isUInt64())) {
    runtime->rtcp_last_report_lsr = payload["rtcp_last_report_lsr"].asInt64();
  }
  if (payload.isMember("rtcp_last_report_dlsr") &&
      (payload["rtcp_last_report_dlsr"].isInt64() ||
       payload["rtcp_last_report_dlsr"].isUInt64())) {
    runtime->rtcp_last_report_dlsr = payload["rtcp_last_report_dlsr"].asInt64();
  }
  ...
  if (payload.isMember("audio_pcm_samples_drained_total") &&
      (payload["audio_pcm_samples_drained_total"].isInt64() ||
       payload["audio_pcm_samples_drained_total"].isUInt64())) {
    runtime->audio_pcm_samples_drained_total =
      payload["audio_pcm_samples_drained_total"].asInt64();
  }
  if (payload.isMember("audio_pcm_samples_owned") &&
      (payload["audio_pcm_samples_owned"].isInt64() ||
       payload["audio_pcm_samples_owned"].isUInt64())) {
    runtime->audio_pcm_samples_owned = payload["audio_pcm_samples_owned"].asInt64();
  }
  if (payload.isMember("audio_last_drain_samples") &&
      (payload["audio_last_drain_samples"].isInt64() ||
  ...
  if (payload.isMember("audio_render_events_total") &&
      (payload["audio_render_events_total"].isInt64() ||
       payload["audio_render_events_total"].isUInt64())) {
    runtime->audio_render_events_total = payload["audio_render_events_total"].asInt64();
  }
  if (payload.isMember("audio_pcm_samples_rendered_total") &&
      (payload["audio_pcm_samples_rendered_total"].isInt64() ||
       payload["audio_pcm_samples_rendered_total"].isUInt64())) {
    runtime->audio_pcm_samples_rendered_total =
      payload["audio_pcm_samples_rendered_total"].asInt64();
  }
  if (payload.isMember("audio_render_window_samples") &&
      (payload["audio_render_window_samples"].isInt64() ||
       payload["audio_render_window_samples"].isUInt64())) {
    runtime->audio_render_window_samples = payload["audio_render_window_samples"].asInt64();
  }
  if (payload.isMember("audio_last_render_samples") &&
      (payload["audio_last_render_samples"].isInt64() ||
  ...
  if (payload.isMember("audio_render_last_error") &&
