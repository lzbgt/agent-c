...
using agentd::openssl_last_error_text;
...
struct EmbeddedTransportState {
  ...
  agentd::OutboundRtpAudioEncoder outbound_audio_encoder;
...
bool generate_dtls_identity(EmbeddedTransportState* engine, std::string* out_err) {
  ...
  std::string subject;
  if (!agentd::generate_ephemeral_dtls_identity(
        &private_key,
        &certificate,
        &fingerprint,
        &subject,
  ...
  engine->dtls_private_key = private_key;
  engine->dtls_certificate = certificate;
  engine->dtls_fingerprint_sha256 = fingerprint;
  engine->dtls_certificate_subject = subject;
...
bool transmit_outbound_audio_rtp(
  ...
  if (!engine->outbound_audio_encoder.encode_20ms(
        pcm,
        pcm_samples,
        sample_rate_hz,
        channels,
  ...
  const uint32_t timestamp_increment = encoded.timestamp_increment;
  std::vector<unsigned char> plain(12 + encoded.payload.size());
  plain[0] = 0x80u;
  ...
  std::memcpy(plain.data() + 12, encoded.payload.data(), encoded.payload.size());
