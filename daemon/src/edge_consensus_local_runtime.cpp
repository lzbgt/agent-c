...
bool run_edge_consensus_local_runtime(
  ...
  transport.post_hello = [db, &cfg](uint64_t* io_seq, std::string* err) {
    return post_hello_local(db, cfg, io_seq, err);
  };
  transport.send_consensus_frame =
    [db, &cfg](
      ...
      return send_consensus_frame_local(db, cfg, frame, raw_target_node_ids, io_seq, err);
    };
  transport.poll_outbox = [db, &cfg](int64_t cursor, Json::Value* out, std::string* err) {
    return poll_outbox_local(db, cfg, cursor, out, err);
  };
