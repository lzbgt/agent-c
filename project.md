...
Do **not** commit provider API keys into this repo.

Use one of:
- environment variables (`OPENAI_API_KEY`, `OPENROUTER_API_KEY`, etc.), or
- the gitignored `project.local.md` (copy from `project.local.md.example`), which is designed for local-only secrets.
