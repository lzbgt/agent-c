...
- Active builtin managed consensus runtimes now also expose a best-effort `runtime.live_status` snapshot while they are
  still running, so status reads can inspect current term, campaign attempts, member/trust identity, and leader
  freshness timing before terminal result is available.
