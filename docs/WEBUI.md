...
- Use the list filter box to match workflow id, trace id, session id, or idempotency key (maps to `q` on `/api/v1/workflows`); it debounces typing and includes a clear button.
