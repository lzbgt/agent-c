...
- 2026-03-15: running `voice_webrtc_peer` start requests are now only reusable when the effective resolved config still
  matches the live runtime; conflicting explicit or config-driven starts, including `runtime_kind=builtin` against a
  live non-builtin peer, now fail closed with `409` plus the existing runtime snapshot.
