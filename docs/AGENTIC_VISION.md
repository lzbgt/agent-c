...
- 2026-03-19: when a live managed runtime is recovered from persisted running state after agentd restart, later stop
  now preserves an explicit terminal signal too, so recovered voice peers and external consensus helpers do not lose
  final-cause fidelity when they are torn down by the new daemon process.
...
4) **Voice workflow spec**
   ...
   - Completed 2026-03-15: session erase now also owns the managed voice runtime lifecycle, stopping the peer and
     removing persisted runtime artifacts (plus owned broker session cleanup when either request-scoped or
     daemon-configured broker credentials are available).
   - Completed 2026-03-19: if session erase has to stop a live voice peer recovered from persisted running state after
     daemon restart, the returned cleanup summary now preserves that peer's explicit terminal signal/result before the
     runtime record is cleared.
...
5) **Node consensus**
   ...
   - Completed 2026-03-20: the builtin managed consensus backend now runs over daemon-local transport instead of
     self-HTTP, so it is materially closer to embedded/node-native adoption and no longer treats `daemon_url` /
     `auth_token` request overrides as semantic runtime config.
   ...
   - Completed 2026-03-20: `GET /api/v1/edge/nodes` now also appends those runtime-backed summaries for managed
     consensus nodes that have no registry row, so list reads no longer lose runtime-only nodes after row deletion or
     before hello/caps registration ever happened.
...
- If daemon voice-backend policy rotates while a managed bundled/external peer is still running, status and
  start-conflict reads now surface `backend_policy_drift.changed_fields[]` plus the current effective start policy
  instead of leaving operators with a generic mismatch.
