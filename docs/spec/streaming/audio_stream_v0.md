...
- When the local build has `libjuice + libsrtp2 + usrsctp` available, the repo now also builds an optional embedded
  ...
  trickled Chromium ICE candidates instead of only relying on the gathered offer SDP. Browser SDP answer
  ...
  rejection, or browser-backed proof for rejected sections plus real/empty candidate ingestion; it is broader browser,
