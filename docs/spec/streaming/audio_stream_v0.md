...
- Valid reserved `runtime_kind=builtin` requests now also return `builtin_start_contract`, which captures the native
  ...
  plus a runtime-schema-shaped `planned_runtime` preview; the same valid builtin `501` response now also exposes that
  preview through top-level `peer`, without pretending the builtin media runtime already exists.
