...
- When the local build has `libjuice + libsrtp2 + usrsctp` available, the repo now also builds an optional embedded
  ...
  audio `msid`/SSRC signaling scoped to audio m-lines when offers contain non-audio sections. The remaining gap is no
  longer basic outbound RTP, Opus ownership, one-shot Sender/Receiver Report transmit, per-frame Sender Report spam,
  compound packet emission, answer direction, or basic browser full-duplex proof; it is broader browser, codec, and
