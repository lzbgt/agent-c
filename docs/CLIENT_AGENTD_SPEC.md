...
- `POST /api/v1/config/update`
  ...
  - provider keys can also be discovered server-side from `.not_in_repo`, `project.local.md`, `~/.env`, or an explicit
    `AGENTD_DOTENV_PATH` (useful when the daemon runs under a different user)
