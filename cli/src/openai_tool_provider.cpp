...
static agent_status_t openai_tool_provider_generate(
  ...
  const std::string content = extract_assistant_content(assistant_msg);
  if (agent_string_set_copy(&out_resp->assistant_content, content.c_str(), content.size()) != AGENT_OK) {
  ...
  if (calls.empty()) {
    (void)set_error(out_resp, "model returned tool calls but none were parseable");
    return AGENT_ERR_INTERNAL;
  }
