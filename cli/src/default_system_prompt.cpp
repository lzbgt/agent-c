...
const char* default_host_system_prompt() {
  return
    ...
    "- If you need client \"world state\" or client-side effects for an autonomous decision, request a bounded client RPC via ui_action(type=\"client_rpc\", rpc_id=..., rpc={kind:..., args:...}) and then wait for client_rpc_result/client_rpc_progress with client_wait_event (or join multiple rpc_ids with client_wait_any/all).\n"
